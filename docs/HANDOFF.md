...
>
> 마지막 갱신: **2026-09-17(밤 · claude-B 종료)** · 5커밋 배포 완료 · 누리 스팟은 wip 브랜치에 반려 상태로 보존
...
## 0-a7. 🔴 **2026-09-18 새벽 2시 · claude-B — 여기가 가장 최신이다**
...
2. **rank-tabbar group 동결** — geometry 는 58px→0 으로 좋아졌는데 화면에서는 **탭바가 사라졌다**.
   `z-index: 1` 로 바꿔 유지 중이며 검증 대기다(아래 열린 것 참고).
...
1. **이용권 '발행' 단위** — 회수는 T 로 통일했다. 발행(장부 발급·시상)은 '장' 그대로 두었다.
   둘 다 T 로 갈지 결정이 필요하다(`StoreDashboard.tsx` 매장이용권 카드).
2. **내 매장 CLS 남은 0.25** — 대시보드 미수금 경고 카드(46px)가 데이터 도착 때 끼어들며 아래를 민다.
   늘 있는 카드가 아니라 자리를 상시 비우면 평소에 빈칸이 보인다. **그대로 두기를 권했다.**
3. **rank-tabbar z-index** 검증 — 되돌리라는 판정이 나오면 `rank-tabbar` 만 group 동결 목록에서 뺀다.
