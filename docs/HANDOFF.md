...
>
> 🔴 **처음 이어받는 사람은 `docs/HANDOVER-2026-09-23.md` 를 먼저 읽어라** — 이 저장소가 반복해서 밟은 실패 10종과
...
> - 닉네임 하나(k): name 은 nickname 의 거울(트리거). 30일 규칙·사칭어 필터·이력(nickname_history)·이용권 받는 사람 검색(실명→닉네임).
> - 모션 통일(e1ea47e8): 하위 탭 25곳·PC 메인 탭이 `tabCover.lift()` 하나. 남은 FAIL: 라이브 첫 방문 + 느린 망에서 빈 덮개 ~540ms(스켈레톤 4장 예약, `LiveGamesTab.tsx:235`).
> - CI 시간 제한 30→45분(807 테스트가 30분에 닿아 취소됐다 — 실패 0).
> - ⏳ 남은 것: 직원 v2(20260924e 보완 중·미적용) → 직원 화면 · 불필요 코드 정리(오너 요청, 미착수) · 연합 리그 초대 알림(화면 없음 — 오너 결정)
>   · 커뮤니티 섹션 첫 방문 덮개(CommunityTab startSecTransition) · PC 매장 열기 View Transition.
> - ⚠ 오늘 두 번 `public/sitemap.xml` 이 팀원의 `npm run build`(Playwright webServer)로 덮어써졌다 — **Playwright 는 `E2E_BASE_URL` 없이 돌리지 마라.**
>
> ✅ **마지막 갱신: 2026-09-23 (Opus 5) — §0-a26 이 정본이다.**
> 🔴 **커뮤니티 '새 글' 알림 표시 2종을 없앴다**(오너 지시) — 행 앞 빨간 점 + 하단 탭 점.
> 생김새가 같은 **대회 Q&A 안읽음 점은 다른 기능이라 살아 있다** — 클래스로 일괄 치환하지 마라(§0-a26 ③).
> ⚠ E2E 전에 **4173 을 잡은 프로세스의 시작 시각**을 확인해라. 어제 남은 preview 가 옛 빌드로 초록을 줄 뻔했다(§0-a26 ④).
>
> 아래는 2026-09-22 기록이다(§0-a25 → §0-a24 순). 그날 §0-a25 **뒤에도 앱 코드 커밋이 2건 더 있다**:
> `6b5fdba2`(포스터 QR 문구를 '참가 신청' 으로 단축) · `8d4341dd`(일정 카드 우측 정렬 + 라이브 생존/엔트리).
> 둘 다 CI 초록·운영 배포 완료이며 별도 절을 쓰지 않았다 — 변경 범위가 좁고 코드 주석에 근거가 남아 있다.
...
## 0-a27. 2026-09-23~24 · Opus 5.5(리드) + 팀 — **오너 요청 14건 · 배포 4묶음 · 라이브 DB 6건 · 엣지 함수 2건** (여기가 가장 최신)

운영 포인터: main `1c77238b`(푸시 완료, CI 진행) · 앞 묶음 `b80391b5`·`b103b091`·`7359bc7c` CI success · 손님 도메인 번들 지문 실측(청크 전이 폐쇄).
진행 규칙: 오너 지시 "다른 부분 오류 없게, 순서는 네가" → 묶음마다 **독립 검증 → 깨끗한 임시 워크트리(node_modules junction + .env.local)에서 gate + 전체 e2e → 푸시 → CI → 라이브 지문**.
E2E 전체 실행의 실패는 전부 **격리 재실행에서 통과**(부하 flake). `mobile-tab-transition` R3 는 **이전 커밋(b103)에서도 1/4 실패** — 기존 flake, 회귀 아님.

### ① 배포된 것
| 커밋 | 요구 | 핵심 |
|---|---|---|
| `b80391b5` | 이벤트 목록 진입 끊김 · 내 정보 바로가기 3열 · 팔로워 알림 | 스켈레톤 1줄·목록 캐시·미리 받기, 열린 오버레이 뒤 탭바 리셋 중지 / 설명줄 제거·보이는 수만큼 균등 / 문구 사실화·실패/매장전환 가드 |
| `f3f2aa30`·`b103b091` | 스타팅 핸드 순위(규칙·대회) · GTO 모드 전환 번쩍임 · 조회수는 상세에서만 | 169핸드 무작위 한 손 상대 승률(공표값 ±0.3%p) / 빌런 보관·서명 캐시·blur 제거·7rem 대칭 상자 / 목록 조회수 제거 |
| `7359bc7c` | 출석 위치 확인(스위치 꺼짐) · 업주 '출석 위치' 칸 · 하단 탭 본문 덮개(기기별 시험) | 아래 ③ |
| `a5154a9b`·`b09df395`·`1c77238b` | NURI SPOT 5단계 작성 화면 · AI 아쉬운 포인트(**꺼짐**) · 처리방침 위탁 고지 | 아래 ④ |

### ② 라이브 DB(전부 롤백 리허설 → 적용 → 파일 머리 ✅ 기록)
`20260923a` 팔로워 알림 하루 3회 KST 자정 · `20260921b` 이용권 사용은 방문만(오너 승인) · `20260923b` check_in 좌표 선택 인자(1단계) ·
`20260923c` SPOT AI 과금 기반(꺼짐) · `20260924a` 관리자 환불 30일 상한에서 AI 자동 환불 분리 + 5분 크론 · `20260924b` 크론 잠금 순서.
엣지 함수: `spot-review` 배포(v3) · **`gto-explain` 410 스텁 배포 — 09-21 BLOCKED 해소**(CLI `npx supabase functions deploy` 가 동작한다. MCP 배포 도구 대신 이걸 써라).

### ③ 출석 위치 확인 — 켜기 전 남은 것
- 오너 결정: 반경 300m + 폰 오차 최대 200m · 오차 1km 초과 거부 · **좌표 없는 매장 차단**.
- 🔴 **매장 좌표 0/4** — 켜면 전 매장 출석이 막힌다. 오너가 매장 '소개' 탭을 열거나 업주 '출석 위치' 칸으로 등록한 뒤에.
- 켜기: `select public.set_app_setting('checkin_geo_enabled','on');` (클라이언트가 좌표 전송 시작) → 1~2일 뒤 3단계(서버에서 좌표 없는 호출 거부, CREATE OR REPLACE).
- NOT_RUN: 실기기 위치 권한(iOS·삼성·카카오 인앱).

### ④ NURI SPOT AI — 켜기 전 남은 것
- 오너 결정: 30포인트 · 하루 3회(KST) · 실패 환불 · 메모 포함(300자) · 개인정보 미전송 · 09-11 "AI 는 TDA 하나" 확장 · 처리방침 고지.
- critical-reviewer 3회 판정: 과금·환불·권한 **PASS**(이중 차감·이중 환불·권한 우회 0). **출력 검사(가짜 수치 차단)만 반복 보강 중** — 금지어 방식 → 허용목록 방식 전환, 4차 수정 진행.
- 🔴 처리방침 개정(v2) 시행일이 **09-29** — 켜는 시점은 오너 결정.
- 켜기: `update shop_skus set active=true where key='spot_ai';` / 끄기 false. 켠 뒤 함수 로그의 '출력 검사 탈락' 사유별 건수로 오탐률 실측.

### ⑤ 오너 기기 확인 부탁(NOT_RUN — 하네스로는 못 잰다)
- 하단 탭 본문 덮개: 삼성 인터넷에서 `https://nuriholdem.com/?fx=tabfade` 로 한 번 연 뒤 탭 이동. 밝기 튐이 없으면 `src/lib/tabCover.ts` `TAB_COVER_DEFAULT_ON=true`. 끄기 `?fx=off`.
- 이벤트 목록 진입(삼성 도구막대 재등장 후보 B 는 실기기로만 판정).

### ⑤-b 2026-09-24 추가 배포(`a095b1e8`·`6af3c97e`·`113c47ae`, CI success · 번들 지문 확인)
- **하단 탭 본문 전환 기본 켜짐(tabsoft)**: 오너 삼성 실기기 '밝기 변화 없음, 딱딱함' → '제일 오류 없는 모션' 지시로 덮개 opacity 만 280ms. 덮개는 **목적지 판이 실제로 그려진 뒤에만** 걷힌다(상한 700ms) — 유휴 미리마운트 직후 lazy 폴백 스로틀로 본문이 317~333ms 늦는 경합을 TC5 로 결정적 재현·음성 대조. `?fx=tabfade` 삭제, `?fx=off` 기기별 끄기. 프로덕션 tab-cover 4회 반복 24/24, 전체 e2e 717 통과.
- **포스터 '예약하기' 누르면 맨 위로(09-13 `1516e2ed` 부터)**: useDialogFocus 포커스 복구 3곳 preventScroll. e2e/dialog-focus-scroll(touchscreen.tap) · 음성 대조 297→0.
- 남은 확인: 오너 폰에서 **주소 없이** 탭 이동해 부드러움 확인(NOT_RUN — 하네스로 못 잰다). 아이폰 Safari 는 탭이 버튼에 포커스를 안 줘 포스터 경로 A 가 원래 없을 수 있다.

### ⑤-c 2026-09-24 오후 배포 2묶음 (`e8b146b3`·`3460c71c`, 둘 다 CI success · 번들 지문 확인)
- 게시글·장터 상세 밀도·장터 카드 · GTO 도구 열기 끊김(Suspense 폴백·blur 키프레임 제거) · **반응 트레이 넘침**(CI 리눅스 글꼴이 ~7px 넓어 09915301 이 CI 실패 → 라벨만 말줄임, 넓힌 글꼴로도 검사)
- 내 매장 모바일 대시보드 헤더 1겹 · `mystore-transition-cls` 면제를 시각→원인(시트 변위 D±2px 1회)으로
- SPOT↔내 스팟 판 유지(단계·스크롤 보존, 삭제가 진행 중 재조회를 무효화) · 내 정보 메뉴 판 유지(대시보드 탭 280→82ms@6×)
- 홈 밀도(일정 제목 15px, 배너 제어를 배너 안으로, 320~412 에서 카드 4장) · **탭 사전 마운트 경합**(App.tsx: 커밋 전 visitedTabs 변이 → 647px LazyFallback 이 푸터 위에 끼어 CLS 0.328, 수정 전 코드에도 있던 결함) · 배너 점 랩 멈춤
- 남은 것: 관리자 배너가 늦게 오면 캐러셀이 4번째 장에서 시작(후보) · 번들 예산 여유 1% · spot-review 소소한 누수 3종 · StoreDashboard 안내 버튼 38px · 출석 위치 3단계(좌표 등록 후 스위치)
- 교훈: 운영 빌드에서 `page.route` 는 서비스 워커가 응답한 /assets 를 못 잡는다(청크 붙잡는 스펙은 `serviceWorkers:'block'`) · 윈도우 로컬 통과 ≠ 리눅스 글꼴 통과(여유 px 를 단언하라)

### ⑥ 팀 운영 교훈(이번에 실제로 밟은 것)
- 워크트리 세션의 팀원은 Edit/Write 가 메인을 거부당해 **워크트리에 쓰고 메인에서 테스트** → 옛 파일 PASS 보고. 위임에 "메인 절대경로에 Bash 로 쓰기"를 박아라.
- 음성 대조 중 공용 체크아웃에서 `git stash -u`·`git checkout --` 가 실제로 나왔다(각자 복구, 해시로 확인). 위임에 금지를 명시.
- 역할 정의 수정은 **같은 세션의 새 spawn 에 즉시 반영**된다(JSONL model·effort 실측). 09-21 '옛 모델로 돈다'는 전역 `fable[1m]` 기본값 탓이었고, 전역 기본은 이제 `claude-opus-5-5`.

---

## 0-a26. 2026-09-23 · Opus 5 — **커뮤니티 '새 글' 알림 표시 2종 제거**

요구 키: 오너 채팅 2026-09-23(사진 1장) — "앞에 빨간 불이 왜 있는지 모르겠고 그리고 커뮤니티 글이
올라올 때마다 하단 커뮤니티 탭에 알림 표시처럼 뜨는 것 없애줘 굳이 있을 필요가 없어"

### ⓪ 표시 두 개는 증상만 비슷하고 **완전히 다른 코드**였다

| | 행 앞 빨간 점 | 하단 탭 점 |
|---|---|---|
| 그리는 곳 | `PostRowCard.tsx` **PostRow**(한 줄 보기 전용) | `App.tsx` **MobileTabBar** |
| 켜지는 조건 | 글이 24시간 이내 | 마지막 커뮤니티 방문 이후 새 글(`nuri:comm-seen`) |
| 색 | danger(빨강) | accent(골드) |
| 들어온 경위 | Phase 14 · pokergosu 목록 밀도 모방(`2d90c658`) | 모바일 앱화 마감(`fba051c6`) |

`git log -S` 로 확인했다 — **둘 다 오너 요청으로 들어온 것이 아니다.** 되살릴 근거가 없다.

⚠ `PostCard`(피드 카드 보기)에는 빨간 점이 **애초에 없었다** — 한 줄 보기(PostRow)에만 있었다.
그리고 `src/lib/boardView.ts` 가 정본인데 **미선택 기본이 `compact`(한 줄 보기)** 다(N08, 2026-09-13).
즉 이 점은 소수가 고른 화면이 아니라 **모든 사용자의 기본 화면**에 떠 있었다.
🔴 조사 에이전트 하나가 `CommunityTab.tsx:590` 의 **낡은 주석**을 근거로 "기본은 feed" 라고 보고했다 — 틀렸다.
보기 모드의 단일 출처는 `boardView.ts` 하나다. 주석은 정본이 아니다.

### ① 빨간 점이 정보량 0 이었던 이유 (오너 "왜 있는지 모르겠고" 에 대한 답)

목록 기본 정렬이 `created_at desc` 다(`src/api/community.ts:666`). 그래서 '24시간 이내' 집합은
**언제나 맨 위 연속 몇 줄**이다 — 위치가 이미 말한 것을 점이 한 번 더 말했을 뿐이다.

운영 실측(2026-09-23 · `community_posts` 상위 20): **7건 중 3건(42.9%)** 이 24시간 이내
= 맨 위 3줄 연속 점등. 오너 사진의 세 줄이 정확히 그것이다.

게다가 색이 danger 였다 — 이 앱에서 빨강은 오류·HOT·비추천이라 '새 글'이 **경고로 읽혔다.**

### ② 지운 것 — 사슬을 통째로. 반쪽 삭제는 빌드가 막는다

| 파일 | 지운 것 |
|---|---|
| `PostRowCard.tsx` | PostRow 의 24시간 판정 + `aria-label="새 글"` 점 |
| `App.tsx` | `commSeenAt` state · `nuri:comm-seen` 방문 기록 `useEffect` · `commHasNew` memo |
| `App.tsx` | `tabDot` memo · `MobileTabBar` 의 `dot` prop(타입·구조분해·렌더) · 호출부 `dot={tabDot}` |

🔴 `tsconfig.app.json` 의 `noUnusedLocals`·`noUnusedParameters` 가 **둘 다 true** 라,
"화면에서만 떼고 상태는 남기는" 반쪽 삭제는 `tsc -b` 에서 죽는다. 그래서 6곳을 한 커밋에 지웠다.
**되살릴 때도 한 덩어리로 되살려야 한다.**

남긴 것: `posts` 조회는 그대로다 — 딥링크 해석(`pendingPostId`)·상세 동기화·좋아요 토글·자식 4곳이
쓰는 별개 소비자다. 각 브라우저에 남은 `nuri:comm-seen` 키는 읽는 코드가 없어 무해하다(정리 코드 안 넣음).

### ③ 🔴 생김새가 같은 **다른 기능**이 둘 있다 — 일괄 치환하면 같이 죽는다

50개 에이전트 전수 조사에서 나온 이번 작업 최대 위험이다.

| 살아 있어야 하는 것 | 자리 | 무엇인가 |
|---|---|---|
| Q&A 안읽음 점 | `ScheduleDetailModal.tsx:401` | 대회 상세 Q&A 탭 안읽음. **클래스 문자열이 빨간 점과 글자 그대로 같다** |
| 라이브 숫자 배지 | `App.tsx` MobileTabBar | '진행 중 N게임'. 커뮤니티 점과 **다른 span** 이고 `count` prop 으로만 켜진다 |

클래스명으로 grep 치환했으면 Q&A 안읽음 표시가 조용히 사라졌을 자리다.
그래서 번들 지문 검사에 **양성 대조**를 같이 넣었다(③ 아래 ④).

불리언 점을 없애고 숫자 배지를 남긴 기준: **'몇 개'를 말하면 누를 이유가 되지만,
'뭔가 있다'까지만 말하는 불리언 점은 이유가 되지 못한다.**

### ④ 검증

번들 지문(`dist/assets/*.js`) — 음성·양성을 같이 쟀다:

| 문자열 | 기대 | 실측 |
|---|---|---|
| `새 글`(행 앞 점 aria) | 사라짐 | 커뮤니티 청크 0 — 남은 1건은 관리자 버튼 문구 `새 글을 써서 N번 칸에 연결`(무관) |
| `comm-seen` | 사라짐 | **0 파일** |
| `새 질문`(Q&A 안읽음) | **남음** | 1 파일 ✅ |
| 라이브 게임 배지 | **남음** | 존재 ✅ |

⚠ E2E 를 처음 돌렸을 때 **어제(09-22 17:07) 남은 `vite preview` 가 4173 을 잡고 있었다.**
Playwright 가 그걸 재사용했으면 **어제 빌드를 검사하고 초록**을 줬을 자리다 — 죽이고 다시 돌렸다.
E2E 전에 4173 점유 프로세스의 **시작 시각**을 확인해라.

로컬 게이트: lint 0오류/491경고 · vitest **2930/268** · build OK · e2e **689 passed / 6 failed**.
그 6건은 전부 **전체 병렬 부하 flake** 였다 — 격리 3회 반복으로 갈랐다:

| 스펙 | 전체 실행 | 격리 ×3 |
|---|---|---|
| `mobile-tab-transition` | 2 실패 | **14/14** |
| `post-nav` | 1 실패 | **10/10** |
| `mystore-transition-cls` | 3 실패 | **6/6** |

CI(`35779139760`)가 같은 스위트를 초록으로 통과시켜 독립 확인이 됐다. 이 머신은 전체 병렬에서 flake 를 만든다.

운영 실측(`dpl_9NRiNVs81zGb…` · entry `index-BeuTCiZN.js` · 3개 도메인 동일):
전이 폐쇄로 **청크 99개**를 받아 검사 → `comm-seen` 0 · `dot?.[` 0 · `새 글` 은 관리자 버튼 문구 1건만.
라이브 DOM(Pixel 7, 커뮤니티→게시판): **행 7개(7/7 카테고리 배지) · 새 글 점 0 ·
하단탭 커뮤니티 표식 0 · 라이브 숫자 배지 `1` 생존**(양성 대조라 프로브가 헛돌지 않았다).

### ⑤ 계약 테스트는 **일부러 안 넣었다**

"이 점이 다시 오면 실패" 계약은 제품 취향 판정이라, `CLAUDE.md` 가 2026-09-07 오너 지시로 삭제한
'스타일을 규정하던 테스트' 부류에 속한다. 재발 방지는 ② 의 빌드 강제 + 4곳 주석 + 이 절로 대신한다.
다시 넣고 싶으면 오너 결정이 먼저다.

---

## 0-a25. 2026-09-22 · Opus 5 — **전 메뉴 밝기 점프의 공통 원인 제거 · PC 내 매장 7칸 PILL · 모바일 일정 시간축** (최신은 위 §0-a26)
...
### 🔴 이어서(9/22 새벽) — 독립 검토 워크플로 결과 반영(재산출) + 카드 위아래 폭 축소

워크플로(모델 회계·수치 타당성 검토 2 → 반증 12 = 14 에이전트, 41분): **확정 5(+정보 2) · 기각 1.** 전부 근거 코드·재계산 첨부(`subagents/workflows/wf_a16a7299-909`).

| 발견 | 판정 | 조치 |
|---|---|---|
| 3인 카드 제거 가중에 `/COMBO[y]` 누락 → 콤보² 과가중(페어 질량 3.6% vs 정확 5.9%) | 확정 medium | `solve-multi.mjs` 수정(Z·오버콜 BR·Q/F 세 곳). 효과: 셔브 −0.2~0.7 · callSB −1.5~2.7%p |
| 수렴 미확인 — 30×300 은 과도기(회차 1 단일 콜러 BR 이 1/outer 로 잔존, 콜 표가 반복수만으로 2~5%p 이동) | 확정 high/medium | burn-in 40 회차 뒤 평균 리셋 + 120×300. **수렴 증거**: 80×300 대비 2단계+ 셀 2/31,941 · 집계 최대 Δ0.34%p |
| ⑤ 교차 검사(빅앤티≥노앤티)의 '모델노앤티' 가 실은 옛 2026-08-20 단일 콜러 표 | 확정 medium | `ante=both` 로 같은 모델 노앤티 산출(검사 전용, 게시 안 함) → ⑤ 위반 40 → **0** |
| 곱 정규화 3인 에퀴티의 손별 편향 ±10%p(페어·Ax +2~7, 수딧 커넥터·브로드웨이 오프수트 −7~10) | 확정 high | **근사 자체의 한계** — '추정' 등급 유지·README 명시. 진짜 3인 에퀴티(169³ MC)로 재산출하면 사라진다 |
| 10↔12bb callSB 이음새: 12bb+ 단일 콜러 표가 BB 오버콜을 무시해 k2·k3 에서 10bb 보다 1~5%p 넓음 | 확정 medium | 미조치(12bb+ 는 오너 범위 밖). ⚠ `check.mjs` 스택 단조성은 **shove 만** 본다 — 콜 표 이음새는 게이트 없음(기록) |
| 3번째 콜러 절단(2~3bb 질량 30~91% 버림 → 2bb 행이 k 에 둔감) · 에퀴티 12k vs 40k 이음새 | 확정 low | 문서화 |
| 오버콜 BR 평균장 근사(π_m 무조건부·potAvg) | **기각** — 영향이 재실행 잡음 이하 | — |

재산출(120×300 · burn 40 · 126표 392초): 셔브 k8 2bb **42.9 → 27.6** · k8 3bb 30.0 → 24.0 · k8 5bb 18.3 → 20.2 · k2 10bb 42.2 → 41.5. `check.mjs` 단조성 0 · 상식 0 · ⑤ 0. `emit` → `nash.data.ts` 189 문자열 재교체 · vitest nash 4 스위트 184 통과.

**카드 위아래 폭 축소**(오너 이미지 지시 "빨간색은 그대로 두고 파란 점선 박스 위아래 폭을 줄여줘"): `ScheduleCard.tsx` 목록 카드 `py-2.5 → py-1.5`, 실측 **89.9 → 81.4px**(390·1440). 같이 바꾼 것 — `--card-h-list` 91→82 · 110→102(<390 은 두 부류 118.5/102.4 − 8.5 의 사이값, 실측 못 함) · `.cv-card-list` cis 패딩 보정 1.25→0.75rem · HomeTab·App 스켈레톤 행 `py` 동기화(스켈레톤 로고 h-16 + 패딩 = 카드). 매장명 링크(빨강)는 17.5px 그대로.

게이트(재산출+카드 축소 빌드, 4173): 대상 8스펙 **80 passed**(theme-tokens-v7 ⑦ CARD-H 390·1440 mode 82.4 vs 토큰 82) · lint 0 · vitest **2912/267** · 전체 e2e **669 passed / 0 failed / 1 flaky(post-nav PC 1440, 재시도 통과) / 11 skipped** · 사이트맵 md5 보존.
...
🔴 **계산만 믿으면 틀렸다.** 390px 산술로는 7+6 이 45.1px 라 통과하지만 **320px 실측은 35.09px** 다(44px 에 9px 모자람 — 경계값 함정). 320px 격자 폭 258.5px 실측(`h-11`·`gap-0.5`): 4열 **63.03** ✅ · 5열 **50.00** ✅ · 6열 41.31 ❌ · 7열 35.09 ❌. → 최소 변경은 **5열 × 3줄(5+5+3)**. 내가 처음 제안한 7+6 은 **틀린 안이었다.**
...
### 🔴 이어진 발견 — "공개 매장이 없다"는 거짓말이었다 (게이트 2개 추가)

§0-a22 의 나머지 skip 2건(`nav-stability.spec.ts` 의 매장 오버레이 검사)은
`test.skip(!opened, '공개 매장이 없어 이 검사는 성립하지 않는다(데이터 조건부)')` 였다.
**사실이 아니었다.** 헬퍼 `openVenue` 가 전체를 `try { … } catch { return false; }` 로 감싸고 있어
**어느 단계가 막혀도 똑같이 false** 를 돌렸고, 호출부는 그걸 '데이터 없음' 으로 읽었다.

실측으로 단계를 하나씩 잠그니 범인이 나왔다 — `[data-sec="venues"] button` 의 `.first()`:

| # | 라벨 | 크기 | 정체 |
|---|---|---|---|
| 0–4 | 전체 · 홀덤펍 · 딜러팀 · 동호회 · 유튜버 | 44×47 | **분류 칩** ← `.first()` 가 이걸 누른다 |
| 5–7 | 로티아레나 · 누누리 테스트 홀덤펍 · 로켓단 | 378×62 | **진짜 매장 카드** |

**공개 매장은 3곳이나 있었다.** 칩을 누르니 매장 페이지가 열릴 리 없고, `catch` 가 삼켜 skip 됐다.
→ `data-testid="venue-card"`(`CommunityTab.tsx`)로 앵커를 잡고,
   `openVenue` 는 **경로가 살아 있는지는 단언으로 잠그고 '매장 0곳' 만 false** 로 돌리게 고쳤다.
   섹션 탭도 라벨('홀덤펍') 대신 `data-testid="sec-tab-venues"` 로 좁혔다 —
   '홀덤펍' 은 **섹션 탭과 분류 칩 두 곳**에 있어 `.first()` 가 엉뚱한 것을 누를 수 있다.

### 깨우니 빨갰다 — 그런데 제품이 아니라 **기대값**이 틀렸다

두 검사 모두 **한 번도 돈 적이 없어 단언이 검증된 적이 없었다.**

- **①「오버레이 위 오버레이 — back 은 한 겹만 벗긴다」**
  `openVenue` 는 홈 → **커뮤니티** → 매장 페이지로 들어간다. 한 겹 벗기면 커뮤니티다.
  그런데 `expect(home).toBe('tab:home')` 이었다 — **back 이 두 겹을 닫으라는 뜻이라 제목과 정면으로 모순**이다.
  (변수 이름 `home` 은 옛날에 홈에서 바로 열던 시절의 흔적이다.)
  → `'tab:community'` 로 정정. **지금 돌고 통과한다.**

- **②「매장 페이지를 연 채 다른 탭으로 이동한 뒤 back」** — 시나리오 자체가 낡았다. 측정:
  · 매장 페이지는 `[0,0,412,839]` **전체 화면 다이얼로그**라 하단 탭바를 덮는다.
  · 탭바 중심 `(206,805)` 의 `elementFromPoint` 가 **다이얼로그 안의 SECTION** 이다.
  · nav 의 `z-index: 50` 이 dialog 의 `40` 보다 **큰데도** 그렇다 —
    **z-index 는 같은 쌓임 맥락 안에서만 겨룬다**(CLAUDE.md 의 `backdrop-filter`/`transform` 메모와 같은 부류).
  즉 `tap()` 의 좌표 누름이 탭바에 닿지 않아 "탭 이동 = 오버레이 닫힘" 이라는 전제가 성립하지 않는다.
  게다가 `openVenue` 가 이미 커뮤니티에 데려다 놓으므로 '커뮤니티로 이동' 은 '다른 탭' 도 아니다.
  → **기대값만 고쳐 초록으로 만들지 않았다.** 그러면 '유령 history 항목' 이라는 잡으려던 결함을
    안 잡는 검사가 된다. `test.fixme(true, …)` 로 **측정값을 붙여 정직하게** 표시했다.
    거짓 이유로 skip 하던 것과는 다르다 — 다음 사람이 무엇을 다시 설계해야 하는지 안다.

🔴 **오너 결정 대기**: 매장 페이지가 열린 동안 **하단 탭바를 누를 수 있어야 하는가?**
현재는 못 누른다(전체 화면 다이얼로그가 덮는다). nav 에 `z-50` 을 준 것은 **위에 두려던 의도**로 보이는데
쌓임 맥락 때문에 효과가 없다 — 죽은 선언이다. 손님 눈에는 탭바가 가려져 보이지 않으므로 **지금 당장 손해는 없다.**
① 현행 유지(덮는 게 맞다 · 죽은 z-50 만 정리) ② 탭바를 위로 올린다(오버레이 위에서도 탭 이동 가능)
— ②를 고르면 ② 검사를 그 계약으로 다시 쓸 수 있다.

### store-destination 2건도 깨웠다 — 그리고 **2주 묵은 표류**가 드러났다 (store-team)

`e2e/store-destination.spec.ts` 는 `loginAs(EMAIL, PASSWORD)` 로 실계정 로그인을 한다.
`e2e/_mockOwner.ts` 의 `bootOwner` 로 갈아타 둘 다 목킹으로 세웠다.

⚠ **'상시 skip' 이라고 쓰면 틀린다 — 리드가 실행 로그로 확인한 정확한 상태는 이렇다:**
· `:26`(지난 미마감 장부 CTA)은 **이 컴퓨터에서는 통과하고 있었다.** describe 의 스킵 조건이
  `!E2E_EMAIL || !E2E_PASSWORD` 인데 통과했다는 것은 `.env.local` 에 **둘 다 있다**는 뜻이다
  (값은 보지 않았다 — `.env.local` 은 deny 규칙으로 막혀 있고 그게 맞다. **관측으로만 추론했다**).
  즉 이 테스트가 꺼져 있던 곳은 **시크릿이 없는 CI** 다. 목킹 전환의 진짜 이득은 거기다.
· `:63`(정산 이동)은 **로컬에서도 skip** 이었다 — 계정 때문이 아니라 아래의 **셀렉터 표류** 때문이다
  (`test.skip(await settleStep.count() === 0, …)`). 그래서 이쪽이 진짜 잠든 게이트였다.
👉 교훈: '계정이 없어서 꺼졌다' 와 '셀렉터가 낡아서 꺼졌다' 는 **로그에서 구분된다.**
  묶어서 한 문장으로 적으면 다음 사람이 엉뚱한 곳을 고친다.
(⚠ 필드 이름은 `extra` 다 — 리드가 `extraRoutes` 라고 잘못 정정했고 팀이 근거를 들고 바로잡았다. `_mockOwner.ts:60`.)

🔴 **② 「정산 이동은 스크롤이 아니라 정산 마감 버튼을 지목한다」 — 셀렉터가 애초에 죽어 있었다.**
원래 `getByRole('button', { name: '정산' })` 를 누르게 돼 있었는데, 그 자리는 `role="tab"` 이고
**2026-09-08 오너 결정으로 '정산' 이 독립 결산 판(LedgerSettlementPanel)으로 승격**됐다
(`VenueManageTab.tsx:73-75` 주석이 그대로 적고 있다: *"5단계 '정산' — 2026-09-08 에 판으로 승격됐다"*).
지금 그 탭을 누르면 전 게임 합산 리포트가 열리고 `ledger-settle` 버튼은 그 화면에 없다.
스펙은 2026-09-07 에 쓰였고 **그 다음날 화면이 바뀌었는데, 계정 은퇴로 한 번도 안 돌아 아무도 몰랐다.**

→ **기대값만 고쳐 초록으로 만들지 않았다.** `settle:true` 시드를 만드는 곳을 전수로 찾으니
   제품 소스에 **단 한 곳**뿐이었다 — `StoreDashboard.tsx:492`('지금 할 일' 카드의 '마감하기' CTA, `:1159`).
   원래 계약을 유지한 채 **지금 유일하게 살아 있는 트리거**로 갈아탔다.

검사는 계약의 **양쪽을 다** 단언한다(공허하지 않다):
`ledger-settle` 이 **보이고 `toBeFocused`** (= '지목한다') · 클릭 전후 `scrollY` 차이 **< 200px** (= '스크롤이 아니라').

**리드 독립 검증**(보고가 아니라 파일·명령으로): 제품 소스 변경 **0**(`git status src/` 빈 출력) ·
`ledger-settle` 은 `NuriPosLedger.tsx:1703` 에 **원래 있던 것** · 워크트리↔main md5 동일 ·
보고 해시 `a2b3fb5b…` 일치 · `tsc -b --force` 0 · `npm run lint` 0오류(773파일) ·
`playwright test e2e/store-destination.spec.ts` **rc=0 · 5 passed / 1 skipped**(남은 1건은 기존 `test.fixme`).

⚠ **별도 결정으로 남긴 것**: GameStepBar 의 '정산' 탭에 `settle` 포커스를 다시 붙일지.
지금은 손대지 않았다 — 그 탭은 '판' 으로 승격된 것이 오너 결정이므로 되돌리는 것은 제품 판단이다.

### 히트영역 전수 감사 — 운영(nuriholdem.com) · 375px · 홈

오늘 `PostComposerExtras` 에서 게이트 사각지대의 44px 미달을 찾은 뒤,
**그게 유일한지**를 손님이 실제로 보는 화면에서 쟀다(로컬이 아니라 운영).

방법: 모든 `button·a[href]·[role=button]·summary` 를 훑고, 각 요소를 화면 안으로 넣은 뒤
중심에서 상하좌우로 1px 씩 넓히며 `elementFromPoint` 가 그 요소(또는 자손)인지 확인해 **실효 히트 영역**을 잰다.
(`.hit` 의 `::after` 오버행은 `getBoundingClientRect` 로 안 잡히므로 박스만 재면 과대 보고가 된다.)

| 항목 | 결과 |
|---|---|
| 44px 미달 | **21개** — 이 화면에는 `.hit` 오버행이 **한 곳도 없다** |
| 실제로 덮여 못 누르는 것 | **0개** |
| 가장 좁은 값 | 가로 **31px**(배너 점 31×35) · 세로 **25px**(푸터 법적 링크) |
| 헤더 아이콘 | 39×39 (5px 모자람) |
| **14-ⓐ 커뮤니티 칩(운영 실측)** | 박스 **44.0×46.8** · 실효 **44~45×48** — ✅ 5개 전부. 덮임 0건 |

⚠ **범위를 넘겨 읽지 마라 — 이 감사는 '비로그인 홈' 한 화면이다.**
`.hit` 자체는 죽은 유틸이 아니다: `src/index.css:1619` 에 정의돼 있고 `App.tsx`·`DateTimePicker.tsx`·
`LoadErrorCard.tsx`·`CalendarPanel.tsx`·`ClockRemote.tsx` 등 **10곳 넘게 쓰인다**(오늘 안 들어간 화면들).
같은 유형으로 `.tap-y-44`(`index.css:1021`, `::before { inset: -6px 0 }`)도 있다.
→ 다른 화면을 감사할 때는 **그 화면에서 다시 재라.** 이 표를 앱 전체로 일반화하지 마라.

🔴 **이 수치가 '글쓰기 랭크 버튼' 결정을 날카롭게 만든다.**
운영 홈에서 가장 좁은 탭 대상이 **31px** 인데 글쓰기 랭크 버튼은 **21.34px** 다 — **앱 안 어디보다 좁다.**
"44px 계약이 원래 안 지켜지지 않느냐" 는 반론으로 넘길 수 없는, **단독으로 예외적인 값**이다.

### ⚠ 이 감사에서 내가 세 번 틀렸다 — 측정 자체가 '아무것도 안 재는' 함정에 빠졌다

1. **화면 밖 좌표**: `elementFromPoint` 는 뷰포트 안에서만 동작한다. 푸터 요소들이 접힌 화면 아래라
   전부 `null` 을 돌려줬고, 내 검사는 그걸 **'미달' 로 위장**했다(실효 `1×1`).
2. **낡은 rect**: 앞 호출에서 `scrollTo(0,0)` 한 뒤의 좌표를 그대로 써서 '뷰포트 안' 이라고 잘못 판정했다.
   `고지 보기` 의 중심은 실제로 **y=931**(뷰포트 812 밖)이었다.
3. **숨김 상태**: `맨 위로` 가 "덮여 있다" 고 나왔는데 실은 `opacity:0 · pointer-events:none` —
   스크롤해야 나타나는 버튼이라 **비활성 상태를 결함으로 읽었다.**

👉 **교훈: 히트영역을 잴 때는 ⓐ 대상이 뷰포트 안인지 ⓑ rect 를 그 순간 다시 읽었는지
ⓒ `opacity`·`pointer-events`·`visibility` 가 활성인지 를 먼저 단언해라.**
셋 중 하나만 빠져도 멀쩡한 화면이 빨갛게 나오고, 반대로 진짜 결함이 묻힌다.
...
#### 그날 저녁에 더 막은 것 (위 우선순위대로 진행했다)

| 파일 | 전 → 후 | 무엇을 잠갔나 |
|---|---|---|
| `api/auth.ts` | 1/6 → **3/6** | `getMyLegalConsents` 의 동의 플래그 4종(**분쟁 시 증거**) · `checkNicknameAvailable`(뒤집히면 **쓰는 닉네임이 '사용 가능'**) |
| `lib/loyalty.ts` | 0/6 → **6/8** | 배지 임계 11종 × 3(정확히 그 숫자 / 하나 적으면 / 넉넉히) · `champion` 은 `=== 1` 만(0·음수도 거절) |
| `api/vouchers.ts` | 1/6 → **2/6** | `iCanViewVouchers` 권한 판정 + **조회 실패를 '권한 없음' 으로 둔갑시키지 않는다**(원문 주석의 과거 사고) |
| `api/ledger.ts` | 1/6 → **2/6** | 이용권·가게지원 바인 금액 0 |
| `api/rankings.ts` | 0/6 → **1/6** | 두 글자 실명 마스킹 |
| `lib/icm.ts` | 5/8 → **8/8** | 경계값 3종 |
| `api/rankverify.ts` | 0/2 → **2/2** | `eventKind`·`isOverseas` 매핑 |

🔴 **배지 테스트에는 '검사 표가 코드보다 뒤처지는 것' 을 막는 장치를 넣었다** —
`이 표가 BADGES 를 전부 덮는가` 를 스스로 검사해, 새 배지를 추가하면 거기서 빨개진다.
오늘 내내 고친 '안 보는 검사' 를 예방하는 쪽이다.

#### ⚠ 생존자 목록을 **그대로 할 일로 옮기지 마라**

- `api/vouchers.ts` 의 L177·L531 `typeof data === 'number' ? data : Number(data)` 는
  **등가 뮤턴트**로 보인다 — 뒤집어도 `Number(5) === 5` 라 동작이 같다.
- `api/marketplace.ts`(0/6)는 손대지 않았다. 생존자 L62~64 가 **목 데이터 분기**
  (`VITE_SUPABASE_URL` 이 없을 때만 도는 로컬 개발 경로)라 운영 동작에 영향이 없다.
  실제 경로의 L169(`ids.length === 0` → 찜 목록이 항상 비어 보인다)만 의미가 있다.
- 스크립트 자신이 적는다: *"전부 고칠 필요는 없다(방어적 분기·불가능한 경로도 생존한다)."*
  **생존자를 보면 먼저 '이 줄이 운영에서 도는가 · 뒤집히면 무엇이 달라지는가' 를 읽어라.**
...
```bash
# ① 운영 index.html 과 엔트리 청크
curl -s https://nuriholdem.com/ -o /tmp/li.html
E=$(grep -oE '/assets/index-[A-Za-z0-9._-]+\.js' /tmp/li.html | head -1); echo "$E"
curl -s "https://nuriholdem.com$E" -o /tmp/le.js -w 'bytes=%{size_download}\n'

# ② 문구가 엔트리에 있나
grep -c '내 활동 등급' /tmp/le.js

# ③ 없으면 지연 청크다 — 로컬 dist 에서 '어느 청크에 사는지' 먼저 찾는다
grep -l '현재 팟(이번 벳 전)' dist/assets/*.js      # → ToolsPanel-XXXX.js
# 그 이름(해시 제외)으로 **운영 엔트리 안에서** 실제 청크명을 찾는다 — 해시는 빌드마다 다르다
grep -oE 'ToolsPanel-[A-Za-z0-9_-]+\.js' /tmp/le.js | sort -u | head -1
curl -s "https://nuriholdem.com/assets/<그 이름>" -o /tmp/c.js -w 'bytes=%{size_download}\n'
grep -c '현재 팟(이번 벳 전)' /tmp/c.js
```

🔴 **반드시 밟는 함정 — `http=200` 을 성공으로 읽지 마라.**
없는 에셋 경로를 요청해도 **SPA 폴백이 index.html 을 200 으로 돌려준다.**
오늘 첫 시도에서 `http=200 · bytes=24648` 을 받았는데, 그 24,648 은 **index.html 과 정확히 같은 크기**였다.
그대로 `grep` 하면 0회가 나와 "배포 안 됐다" 는 **거짓 결론**이 난다.
👉 **받은 바이트 수를 index.html 크기와 비교해라.** 같으면 그 청크는 운영에 **없는 것**이다(= 이름이 틀렸다).

🔴 **엔트리에 없다고 '배포 안 됨' 이 아니다.** 이 앱은 화면별로 청크를 쪼갠다.
오늘 6종 중 **엔트리에 있던 것은 2종뿐**이고 나머지 4종은 각자의 화면 청크에 있었다.
...
| 15-ⓐ | `시작 시각 지남` | 엔트리 |
| 19-ⓒ | `내 활동 등급` | 엔트리 |
| 29-ⓐ | `현재 팟(이번 벳 전)` | `ToolsPanel`(181KB) |
| 6-ⓑ | `플러스` | `CalendarPanel`(27KB) |
| 25-ⓐ | `같은 시간대 일정 중에서` | `VenueManageTab`(424KB) |
| 24-ⓐ | `다른 기기에서도 함께 로그아웃됩니다` | `CustomerDashboardPage`(93KB) |

**화면 동작까지 봐야 하는 것**은 브라우저로 실제 화면에서 잰다 — 예: 14-ⓐ 커뮤니티 칩은
운영에서 박스 **44.0×46.8** · 실효 **44~45×48** · 덮임 0건을 `elementFromPoint` 로 확인했다.
번들에 문자열이 있다는 것과 **화면에서 그게 보이고 눌린다**는 것은 다르다.


### vitest 2831개도 훑었다 — **소스를 스캔하는 계약 테스트 2건이 조용히 꺼질 수 있었다**

E2E skip 을 전수 점검한 뒤 같은 눈으로 vitest 를 봤다.
대부분의 `toEqual([])`(113건)·`toBe(0)`(154건)은 **순수 함수 테스트**라 입력을 테스트가 직접 만든다 — 공허할 수 없다.
위험한 것은 **소스 트리를 훑어 "위반 0건" 을 단언하는 계약 테스트**다(25개). 훑은 파일이 0개면 그냥 통과한다.

25개 중 **파일 수 바닥을 단언하지 않는 것이 5개**였고, 그중 **2개가 실제로 위험**했다:

| 파일 | 조용한 탈출 | 지키던 것 |
|---|---|---|
| `components/features/opsAnalysisSurface.test.ts:25` | `try { readdirSync } catch { return out }` | **오너 지시 2026-09-11** — 인력 부족·증원 추천 금지 |
| `api/aiSurface.test.ts:87` | `if (!existsSync(FUNCS)) return;` | Gemini 호출은 `tda-assist` 뿐 · 옛 `gemini` 는 410 스텁 |

나머지 3개(`rankverify`·`noExternalExport`·`regClose`)는 `readdirSync` 를 **감싸지 않아** 경로가 깨지면 크게 터진다 — 그대로 둔다.

🔴 **총 개수 바닥 하나로는 못 막는다.** `opsAnalysisSurface` 는 `src`(615개)와 `supabase/functions`(10개)를
같이 훑는데, functions 가 통째로 사라져도 **615 + 0 = 615** 라 어떤 바닥도 통과한다.
→ **경로별로** 단언했다.

**음성 대조(색이 아니라 어느 줄이 어떤 메시지로):**
- `opsAnalysisSurface`: `FUNCS` 를 없는 경로로 바꾸니 새 플로어가
  `supabase/functions 를 한 파일도 못 읽었다 — 엣지 함수 쪽 계약이 통째로 꺼져 있다` 로 정확히 터졌다.
  🔴 **그리고 같은 실행에서 기존 검사 2개는 여전히 초록이었다** — 취약점이 실재했다는 증거다.
  (즉 엣지 함수가 사라져도 오너 지시 계약이 그쪽에서 조용히 꺼진 채 CI 는 초록이었을 것이다.)
- `aiSurface`: 같은 조작에 `supabase/functions 가 없다 — Gemini 호출 계약이 통째로 꺼진다` 로 터졌다.
  ⚠ 참고로 그 안의 `expect(callers.sort()).toEqual(['tda-assist'])` 는 **양성 단언**이라 스스로를 지킨다
  (빈 스캔이면 `[] !== ['tda-assist']`). 문제는 87행이 **그 전에 전체를 중단**시킨 것이었다.

### ⚠ 이 작업에서 내가 또 틀렸다 (두 번)

1. **한 가지 모양만 찾았다.** 처음엔 `catch { return` 만 grep 해서 `opsAnalysisSurface` 하나만 찾았다.
   `aiSurface` 의 `if (!existsSync(FUNCS)) return;` 은 **같은 결함인데 모양이 달라** 빠졌다.
   👉 조용한 탈출은 `catch{}` · `if (!x) return` · `if (!x) continue` · `test.skip(조건)` 로 **여러 모양**이다.
     하나만 grep 하고 "전수" 라고 쓰지 마라.
   👉 다만 **모든 `continue` 가 결함은 아니다** — 스캐너의 정상적인 필터링(면제 파일·비대상 줄)과,
     **검사 전체를 중단하는 것**을 구분해라. 후자만 고쳤다.
2. **음성 대조 백업이 수정 전 파일이었다.** 복원했더니 내 수정이 같이 날아갔다.
   복원 직후 `grep` 으로 "수정이 살아 있나" 를 확인하는 줄을 넣어 둬서 잡았다.
   👉 **음성 대조 백업은 '수정 완료 직후' 에 떠라.** 그리고 복원 뒤에는
     '조작이 지워졌나' 와 '내 수정이 남았나' 를 **둘 다** 확인해라.


### 남은 skip 12건을 전수 점검했다 — **더 이상 거짓 이유는 없다**

| 분류 | 건수 | 판정 |
|---|---|---|
| 계측 전용 (`NURI_PERF` · `NURI_MEASURE` 일 때만 실행) | 4 | ✅ 의도된 것 |
| 로그인·E2E 계정 의존 (클락 3 · 내 매장 1 · 손님 대시보드 2) | 6 | ⚠ 아래 |
| 정직하게 `test.fixme` 로 표시됨 | 2 | ✅ |

🔴 **깨우려다 실패한 2건 — 내 가정이 반대였다.** `ad-click-paths:133`('광고 문의' 는 AD 가 아니다)과
`auth-layout:145`(signup-owner 직접 진입)에 `stubLogin` 을 심어 봤는데 **오히려 항목이 사라졌다.**
소스 주석이 답이다 — `CustomerDashboardPage.tsx:824`:
> *1:1 문의 모달은 로그인 전용이라 **비로그인 랜딩엔 mailto***

즉 그 블록은 **비로그인 갈래**에 있고, 로그인하면 다른 갈래가 그려진다.
그런데 실측하면 **비로그인으로는 그 화면에 갈 수가 없다**: `/?tab=profile` 로 딥링크하면
본문이 **홈**이다(`홈로그인홈일정 탐색라이브…`). 프로필 탭이 로그인 전용이기 때문이다.
→ 이 두 검사는 **'비로그인 손님 대시보드 랜딩' 이라는 화면에 도달하는 경로**를 먼저 찾아야 깨어난다.
  `stubLogin` 은 답이 아니다. 변경은 **되돌렸고**(`git checkout --`), 스펙은 손대지 않았다.

⚠ **참고**: E2E 계정이 은퇴해서 6건이 꺼져 있는데, 오늘 store-team 이
`e2e/_mockOwner.ts` 의 `bootOwner` 로 **계정 없이** 내 매장 PC 화면을 검사하는 스펙을 만들었다.
같은 방법으로 `store-destination` 2건은 되살릴 수 있다.
🔴 다만 **클락 3건(`clock-catchup`·`clock-watchdog`)은 목킹하면 안 된다** —
그 검사들은 `clock_states` 가 **실제로 전진하는지**를 보는 것이라, 응답을 목킹하면 검사가 통째로 무의미해진다.
`E2E_ALLOW_WRITES=1` + 비운영 프로젝트가 있어야 한다(현재 운영 ref 는 쓰기 차단이 의도다).
