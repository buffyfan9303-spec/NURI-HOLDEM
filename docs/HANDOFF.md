...
| 무엇 | 커밋 |
|---|---|
| 내 매장 "드르륵" 근치(공통 레시피에 태움) · `useDeferredValue` 3개 제거 | `e0a3694` |
| `20260915i` 급여 이름축 제거 · `20260915j` 초대 권한 + fail-open 제거 | `e0a3694` |
...
**서버만 열려 있던 직원 권한을 화면에 잇는 중이다.** 마이그레이션 g·h·i·j 는 이미 라이브인데
화면이 없으면 **아무도 쓸 수 없다**(권한을 줄 버튼도, 직원이 자기 급여를 볼 곳도 없었다).

건드린 파일(전부 미커밋일 수 있다):
- `src/lib/staffAccess.ts` — `AccessKind` 에 `'schedule'` 추가 + 라벨
- `src/api/ledger.ts` — `getScheduleAccessUserIds`/`grantScheduleAccess`/`revokeScheduleAccess`
- `src/api/staffSchedule.ts` — `getMyStaffWage()`(RPC `my_staff_wage`)
- `src/components/features/VenueManageTab.tsx` — 직원 목록에 **스케줄 편성 권한 버튼** 추가
  (상태·조회 effect·토글은 기존 장부/이용권과 **같은 모양**. 셋이 복사본이라 `ponytail:` 주석을 남겼다 —
   네 번째가 생기면 묶어라)
- `src/api/auth.ts` — `VenueInvite` 에 grant 4필드 + `setInviteGrants()`
  ⚠ 이 파일은 **LF** 다(대부분의 tsx 는 CRLF). 편집 스크립트에서 줄바꿈을 섞지 마라 — 오늘 한 번 섞었다
- `supabase/migrations/20260915k_*.sql` — 초대 목록 RPC 가 grant 값을 내려주도록 확장
  🔴 반환 타입 변경이라 **DROP + 재생성**이고 그때 **ACL 이 초기화된다.** 원래 값(anon·authenticated·service_role)을
  파일 안에서 그대로 복원하고 자가검사로 셋 다 확인한다 — 하나라도 빠지면 화면이 조용히 죽는다
- `src/components/features/StoreDashboard.tsx` — 권한 0 직원의 **막다른 안내 화면**을
  `MyStaffCard`(본인 시급·급여일·휴무 + 출근 관리 안내)로 교체. 오너 요구 ④ 의 나머지 반쪽이다.
  ⚠ 0행과 오류를 **갈라 말한다** — 0행은 "업주가 아직 연결 안 함"(정상)이고 오류는 조회 실패다.

이전 단계에서 건드린 파일 넷(이미 커밋됨 `e0a3694`):
...
### 🔴 아직 **안 된** 직원 권한 항목 — 오너 요구 대비

| 오너 요구 | 상태 |
|---|---|
| ① 초대 시 직급·직함·권한 부여 | ✅ **완료** — 대기 중 초대 행에서 직책 + 장부·이용권·스케줄을 미리 켠다(`20260915j`·`k`). 수락과 **같은 트랜잭션**에서 부여되므로 "들어왔는데 아무것도 못 하는" 구간이 없다 |
| ② 장부만 / 정산까지 / 스케줄 편성 | 장부·이용권·**스케줄** 3종 완료. **정산 분리는 서버에서 불가**(아래) |
| ③ 권한 있는 탭만 보인다 | **원래 맞게 짜여 있었다**(`VenueManageTab` 의 `caps`) — 문제는 순전히 서버였고 그건 닫았다 |
| ④ 본인 스케줄·인건비·출퇴근 | 스케줄·출퇴근은 원래 있었다. **인건비 완료**(이번 작업) |
| 직함 ↔ 권한 연결 | **안 함.** `set_staff_title` 이 20자 **자유 텍스트**를 화이트리스트 없이 저장한다 — 자유 문자열에 권한을 매달면 "매니저"/"매니져" 오타가 조용히 접근을 바꾸는 **fail-open 설계**가 된다. 권고안: **직함은 표시용, 권한은 명시적 버튼, 직함은 그 버튼을 채우는 프리셋**. 오너 승인 필요 |

🔴 **"정산까지" 는 서버에서 가를 수 없다 — 화면만 숨기지 마라.**
`src/lib/ledgerSettlement.ts` 의 `settlementReport(date, sessions, buyins, players, …)` 입력이
`ledger_sessions`·`ledger_buyins`·`ledger_players` **셋뿐**이고, 세 테이블 SELECT 정책이 전부 `can_access_ledger` 다.
**장부 권한을 준 순간 정산 숫자를 이미 준 것**이다 — 그 사람은 콘솔에서 같은 세 테이블을 읽어 같은 함수를 돌리면 된다.
settle 탭만 숨기면 2026-09-15 에 닫은 "화면이 유일한 가드" 5개와 **똑같은 부류를 새로 만드는 것**이다.
진짜로 가르려면 장부 읽기 경로(`src/api/ledger.ts` + `NuriPosLedgerM`)를 RPC 로 **재작성**해야 한다 —
마이그레이션 범위가 아니다. 오너에게 (a) 화면 정리로 충분한지 (b) 서버 차단이 필수인지 먼저 물어라.
