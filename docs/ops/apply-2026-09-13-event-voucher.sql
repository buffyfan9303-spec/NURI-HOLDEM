-- ============================================================================
-- NURI HOLDEM — 이벤트 + 이용권 적용 묶음 (2026-09-13)
--
--   STATUS: PART 1 APPLIED (2026-09-13).
--   FOLLOW-UP APPLIED: 20260913132414_voucher_game_session_lock.
--   PART 2 BLOCKED: event_campaigns has 0 rows. DO NOT RERUN PART 1.
--
--   ⚠ 이 스크립트는 begin/commit 으로 감싸 **전부 적용되거나 전부 취소**된다.
--     중간에 하나라도 실패하면 DB 는 지금 상태 그대로 남는다.
--
--   포함된 마이그레이션 6개 (파일명 순 = 적용 순서, c→d 순서는 반드시 지켜야 한다):
--   1) 20260911g_voucher_multi_use_pending_uniq       이용권 2장 사용 — 대기 유니크를 voucher_id is null 로 좁히고 lbr_insert_self 위조를 막는다
--   2) 20260911i_bulk_delete_voucher_restore          일괄 삭제(플레이어·세션) 두 경로에서 이용권을 복구한다 — 영구 소멸 차단
--   3) 20260912a_buyin_request_approve_race           승인↔거절 경합 — 승인이 거절을 덮어 이용권 1장으로 2회 참가하던 것
--   4) 20260912b_voucher_qr_game_seq                  이용권 QR/전화 사용 — 비로그인 fail-open 차단 + 게임 회차(p_game_seq) 전달
--   5) 20260912c_admin_event_ops                      관리자 이벤트 RPC 8종 + store_vouchers 이벤트 원천참조 2컬럼  ⚠ d 보다 먼저
--   6) 20260912d_event_visibility_and_menu            이벤트 숨기기 축(hidden_at) + 기간 경계를 [starts_at, ends_at) 로 정정  ⚠ c 다음
--
--   근거: 2026-09-13 적대적 검증. 적용 시점에 기존 행을 바꾸는 문장 0건,
--         참조 객체 30개 전부 이미 적용됨, 자가검사 ABORT 조건 0건.
--
--   적용 후에는 아래 PART 2·PART 3 을 **따로** 실행한다.
-- ============================================================================

-- ┌────────────────────────────────────────────────────────────────────────┐
-- │ PART 1 — 마이그레이션 6개 (여기부터 'PART 1 끝' 까지 한 번에 실행)      │
-- └────────────────────────────────────────────────────────────────────────┘
...
-- ── PART 1 끝 ───────────────────────────────────────────────────────────────
commit;
-- 여기까지 오류 없이 끝났으면 6개가 전부 적용됐다.


-- ┌────────────────────────────────────────────────────────────────────────┐
-- │ PART 2 — 매장 이용권 발급 한도 (이벤트를 '공개' 하려면 필요하다)        │
-- │            PART 1 이 성공한 뒤 따로 실행한다.                           │
-- └────────────────────────────────────────────────────────────────────────┘
--
-- 왜 필요한가: 서버가 공개 직전에 '필요한 이용권 37장 ≤ 매장 발급 한도' 를 확인한다
--   (20260912c:234). 한도 기본값은 0 이고(baseline:883), 한도를 올리는 UI 가 앱에 없다.
--   admin_grant_voucher_quota 는 한도 가산과 동시에 voucher_issue_approved=true 까지 세운다.

-- ① 대상 매장과 현재 한도를 확인한다 (읽기 전용)
select c.slug, c.title, c.status,
       c.starts_at, c.ends_at,                       -- 둘 다 null 이어야 '무기한'
       v.id as venue_id, v.name as venue_name,
       v.voucher_quota, v.voucher_issue_approved,
       (select count(*) from public.event_cards ec where ec.campaign_id = c.id) as cards,
       (select coalesce(sum(ec.voucher_count), 0) from public.event_cards ec
         where ec.campaign_id = c.id) as vouchers_needed
  from public.event_campaigns c
  join public.venues v on v.id = c.venue_id
 where c.slug = 'card-open-2026-09';

-- ② 위 결과의 venue_id 와 vouchers_needed 를 넣어 한도를 올린다
--    (vouchers_needed 는 37 이 정상이다. 여유를 두어 40 을 준다.)
-- select public.admin_grant_voucher_quota('<위에서 나온 venue_id>'::uuid, 40);


-- ┌────────────────────────────────────────────────────────────────────────┐
-- │ PART 3 — 적용 후 확인 (읽기 전용 · 따로 실행)                           │
-- └────────────────────────────────────────────────────────────────────────┘

-- ① 이번에 만들어졌어야 할 함수 12종이 전부 있는가 (12행이 나와야 정상)
select p.proname,
  ...
  from pg_proc p join pg_namespace n on n.oid = p.pronamespace
 where n.nspname = 'public'
   and p.proname in ('admin_list_event_campaigns','admin_create_event_campaign',
                     'admin_compose_event_cards','admin_validate_event_campaign',
                     'admin_publish_event_campaign','admin_end_event_campaign',
                     'admin_delete_event_draft','admin_set_event_campaign_hidden',
                     '_event_campaign_problems','event_board','open_event_card',
                     '_grant_event_tickets')
 order by p.proname;

-- ② 이용권 RPC 가 3인자(게임 회차 포함) 한 개씩만 남았는가 (각 1행 = 옛 2인자 제거됨)
select p.proname, pg_get_function_identity_arguments(p.oid) as args
  from pg_proc p join pg_namespace n on n.oid = p.pronamespace
 where n.nspname = 'public'
   and p.proname in ('redeem_my_voucher_by_qr','redeem_my_voucher_by_phone')
 order by 1, 2;

-- ③ anon 에게 열려 있으면 안 되는 것들 — **0행이 정상**
select p.proname
  from pg_proc p join pg_namespace n on n.oid = p.pronamespace
 where n.nspname = 'public'
   and p.proname in ('admin_list_event_campaigns','admin_create_event_campaign',
                     'admin_compose_event_cards','admin_publish_event_campaign',
                     'admin_end_event_campaign','admin_delete_event_draft',
                     'admin_set_event_campaign_hidden','_event_campaign_problems',
                     'open_event_card','redeem_my_voucher_by_qr','redeem_my_voucher_by_phone')
   and has_function_privilege('anon', p.oid, 'execute');

-- ④ 이용권 2장 문제를 닫은 인덱스가 실제로 좁혀졌는가
--    (indexdef 에 'voucher_id IS NULL' 이 들어 있어야 한다)
select indexname, indexdef
  from pg_indexes
 where schemaname = 'public' and indexname = 'ledger_buyin_req_uniq_pending';

-- ⑤ 위조 티켓 요청을 막는 정책이 좁혀졌는가 (with_check 에 voucher_id is null)
select polname, pg_get_expr(polwithcheck, polrelid) as with_check
  from pg_policy
 where polrelid = 'public.ledger_buyin_requests'::regclass and polname = 'lbr_insert_self';

-- ⑥ 이벤트 컬럼 2개와 FK 가 붙었는가
select column_name, data_type
  from information_schema.columns
 where table_schema = 'public' and table_name = 'store_vouchers'
   and column_name in ('event_campaign_id','event_card_idx')
 order by 1;

-- ⑦ 캠페인 상태 — 공개 전에는 status='draft', starts_at·ends_at 이 둘 다 null 이어야 한다
select slug, title, status, starts_at, ends_at, hidden_at, ticket_venue_id
  from public.event_campaigns where slug = 'card-open-2026-09';

-- ⑧ 카드판 — 1|1|10 · 2|2|10 · 3|3|6 · 4|11|11 · null|83|0 이 나와야 정상
select tier, count(*) as cards, sum(voucher_count) as vouchers
...
-- ⑨ 킬스위치 현재 값 — 아직 켜기 전이면 행이 없거나 'off' 다
select key, value, updated_at from public.app_settings
 where key in ('identity_voucher_enabled','event_menu_visible');
