...
// 그래서 이 스펙은 이제 네 가지를 **같이** 잰다 — 하나라도 빠지면 등급 경계가 밀려도 초록이 뜬다:
//   ① 2~10bb(뒤 2명 이상)도 눈금이 '있음' 이고 실제로 행렬이 그려진다(색칠된 셀이 있다)
//   ② 그 구간의 출처 배지는 '추정' 이라고 말한다 — 12bb 이상은 말하지 않는다(등급이 다르다)
//   ③ 경계 — **11bb 는 눈금에 없으므로 10bb ↔ 12bb 가 바로 이웃**이다. 눈금 하나 차이로 등급이 갈리는 것을 직접 누른다
...
const APPROX = [2, 3, 4, 5, 6, 7, 8, 9, 10];   // 빅앤티 k≥2 추정 구간 — nash.data.ts 의 NASH_ANTE_APPROX 와 같아야 한다
...
test.describe('푸시·폴드 눈금 — 깊이의 등급을 미리 말한다', () => {
  ...
  test('🔴 BTN 은 2~10bb 가 추정 등급으로 그려지고 12bb 부터 정식 등급이다 · SB 는 2bb 부터 정식', async ({ page }) => {
    ...
    // ① 기본 BTN(뒤 2명): 전 깊이 '있음' — 점선(없음) 표시가 남아 있으면 추정값이 안 실린 것이다
    for (const s of [...APPROX, ...EXACT]) {
      await expect(tick(s), `${s}bb 눈금이 '없음' 으로 표시됐다 — 추정값(NASH_ANTE_APPROX)이 안 실렸다`).toHaveAttribute('data-has-data', 'true');
    ...
    // ② 추정 구간 — 눌린다 → 행렬이 실제로 그려지고 배지가 '추정' 이라고 말한다
    await tick(5).click();   // 실제 클릭(액션 가능성 검사 포함) — dispatchEvent 로 우회하면 '못 누르는' 회귀를 놓친다
    await expect(dlg.getByTestId('pushfold-no-data'), '5bb BTN 에 안내 상자가 떴다 — 추정값이 안 읽힌다').toHaveCount(0);
    await expect(cells, '5bb BTN 행렬이 없다').toHaveCount(169);
    expect(await colored(), '5bb BTN 행렬에 색칠된 셀이 없다(전부 0 = 전부 폴드)').toBeGreaterThan(20);
    await expect(source, '5bb BTN 배지가 추정 등급을 말하지 않는다').toHaveAttribute('data-approx', 'true');
    await expect(source).toContainText('추정');

    // ③ 경계 — 눈금 하나 차이(10bb ↔ 12bb). 11bb 는 NASH_STACKS 에 없어 이 둘이 바로 이웃이다.
    await tick(10).click();
    await expect(source, '10bb BTN 이 추정 등급이 아니다 — 경계 상한이 밀렸다').toHaveAttribute('data-approx', 'true');
    expect(await colored(), '10bb BTN 행렬에 색칠된 셀이 없다').toBeGreaterThan(20);
    await tick(12).click();
    await expect(source, '12bb BTN 이 추정 등급으로 표시됐다 — 경계가 밀렸다').toHaveAttribute('data-approx', 'false');
    await expect(source).not.toContainText('추정');
    expect(await colored(), '12bb BTN 행렬에 색칠된 셀이 없다').toBeGreaterThan(20);

    // ①-2 정식 등급 나머지 깊이도 실제로 그려진다(속성만 true 이고 빈 표면 여기서 걸린다)
