...
// 섹션 변경·회전·새로고침·뒤로가기 × 360/412px)를 돌려도 어긋남 0건이라 **원인을 특정하지 못했다.**
// 그래서 '원인을 막는' 대신 '증상이 남아 있을 수 없게' 만들었다(SlidingPill 의 MutationObserver +
// visibilitychange + 3·5초 타이머). 이 스펙은 그 그물이 실제로 도는지를 잠근다.
//
