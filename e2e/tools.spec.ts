...
test.describe('도구 탭 — 전체화면 실행', () => {
  ...
  test('🔴 푸시폴드 — 자체 Nash 데이터가 포지션·스택별로 바뀐다', async ({ page }) => {
    ...
    // 기본 BTN **10bb** — 상식 범위(아래 단언은 15~50).
    // ⚠ 2026-09-19: 기본 깊이가 10 → 12 로 바뀌었다가(10bb 이하 k≥2 가 격리되면서 기본 화면이
    //   빈 상자가 됐기 때문) 2026-09-26 오너 지시로 12 → 10 으로 되돌아갔다 — 10bb 는 이제
    //   allowApprox 로 채워져 빈 상자가 아니다(PushFoldChart.tsx 상단 주석 참고). **이 단언들은 손대지 않았다.**
    //   범위 검사(15~50)는 정확한 bb 값에 의존하지 않아 12→10 전환에도 그대로 유효하다.
