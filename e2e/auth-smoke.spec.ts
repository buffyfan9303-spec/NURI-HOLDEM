...
//      ① CI 에서 자격증명이 없으면 리포트에 경고(annotation)를 남기고 skip 한다 — 2026-09-10 까지는 **실패**였는데,
//         그날 오너가 테스트 계정 전부와 시크릿 4개를 지워 '없음'이 의도된 상태가 됐다(조용한 skip 은 여전히 아니다).
...
test.describe('인증 스모크', () => {
  ...
  test('자격증명 설정 확인(CI 필수)', async () => {
    // 2026-09-10 오너 결정: 런칭 전 테스트 계정을 전부 지웠고(E2E 계정 포함) 시크릿 4개도 삭제했다.
    // 그래서 CI 에서도 자격증명 부재는 이제 '사고'가 아니라 **의도된 상태**다 — 실패 대신 눈에 보이는 skip 으로 남긴다
    // (조용한 skip 을 막는다는 원래 의도는 아래 주석(annotation)이 리포트에 남는 것으로 지킨다).
    // 로그인 스펙을 다시 살리려면: 전용 테스트 계정 생성 → E2E_EMAIL/E2E_PASSWORD(+E2E_CLOCK_VENUE/E2E_WATCHDOG_VENUE) 재등록.
    if (!HAS_CREDS && process.env.CI) {
      test.info().annotations.push({ type: 'warning', description: 'E2E_EMAIL/E2E_PASSWORD 가 CI 에 없다 — 로그인 스펙 12개가 skip 된다(2026-09-10 E2E 계정 은퇴, 오너 결정).' });
    }
    test.skip(!HAS_CREDS, '자격증명 미설정 — 공개 스모크만 실행(E2E 계정 은퇴 상태)');
