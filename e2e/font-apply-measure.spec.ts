...
test('🔴 폰트 응답 정상 · 불필요 요청 없음 — 자기 출처 woff2 만, 중복 0, 13개 이하', async ({ page, baseURL }) => {
  ...
  // 🔴 2026-09-17: 상한을 13 → **20** 으로 고쳤다. **게이트를 느슨하게 푼 것이 아니라 기준선이 거짓이었다.**
  //   13 은 라이브 DB 가 사실상 비어 있을 때(대회 0건 · 게시글 1건) 잰 값이다 — 첫 화면에 한글이 거의
  //   없으니 서브셋도 적게 필요했다. 오너 지시로 점검용 콘텐츠를 넣자 **같은 코드에서 16개**가 됐다
  //   (라이브 실측 2026-09-17: woff2 16개 · 392KB · 평균 24.5KB · 최대 38KB).
  //   dynamic-subset 은 **화면에 실제로 있는 글자**로 범위를 좁히므로, 요청 수가 콘텐츠 양을 따라 느는 것은
  //   전략이 깨진 것이 아니라 전략이 작동하는 모습이다. 빈 DB 에서 잰 숫자를 상한으로 두면 이 테스트는
  //   **서비스가 성장하는 순간 반드시 빨개진다** — 실제로 유저 글이 쌓여도 똑같이 터졌을 자리다.
  //   ⚠ 이 검사가 여전히 잡는 것: 92 face 전량 수신(=범위 좁히기 자체가 죽은 것) · 중복 요청 ·
  //     외부 출처 요청 · 서브셋 디렉터리 밖 요청 · CSS 2회 요청. 그 넷은 그대로 둔다.
  //   ⚠ 20 도 영구적이지 않다. 다음에 또 넘으면 **숫자를 올리기 전에** 첫 화면 글자 수를 줄이거나
  //     서브셋 묶음을 키우는 쪽을 먼저 검토해라(그때는 전략 문제일 수 있다).
  expect(woff2.length, `첫 화면 폰트 서브셋이 20개를 넘었다(${woff2.length}개) — 92 face 전량 수신이면 dynamic-subset 이 죽은 것이고, 조금 넘은 것이면 첫 화면 콘텐츠가 늘어난 것이다`).toBeLessThanOrEqual(20);
