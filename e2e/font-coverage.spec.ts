...
test.describe('폰트 커버리지 — 한글이 깨지지 않는다', () => {
  // ⚠ 2026-09-12 이름 정정: 예전 제목은 "Pretendard **자신이 그린다**" 였는데, 이 스펙은 CSS 를 직접
  //   fetch 해 FontFace 로 **스스로 등록한 뒤** 재기 때문에 **앱이 폰트를 쓰는지와 무관하다**.
  //   실제로 앱에서 Pretendard 로딩을 멈춘 뒤에도 그대로 통과했다 — 제목이 검증하지 않는 것을 말하고 있었다.
  //   이 스펙이 지키는 것은 **서브셋 자산의 글자 커버리지**다(서브셋을 좁히면 즉시 실패한다).
  //   '앱이 실제로 무슨 폰트로 그리는가' 는 `e2e/font-apply-measure.spec.ts` 가 CDP 로 따로 못 박는다.
  test('🔴 서브셋 자산에 한글·기호가 들어 있다 — 두부(□)로 떨어지지 않는다', async ({ page }) => {
