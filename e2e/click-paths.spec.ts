...
test('이벤트 딥링크 ?event=1 — 이벤트 **별도 페이지**가 열린다(탭이 아니라)', async ({ page }) => {
  ...
  //   옛 딥링크 `?event=1` 은 그대로 받아 **지금 열려 있는 캠페인**의 slug 로 승격한다(QR·공유 링크 보존).
  //
  // ⚠ 계약 변경(2026-09-15): 예전엔 여기서 `'card-open-2026-09'` 라는 **고정 slug** 를 단언했다.
  //   그런데 진입은 더 이상 고정 캠페인을 열지 않는다 — '지금 진행 중인 캠페인'을 연다(운영 DB 에
  //   그 고정 캠페인은 행조차 없었고, 그래서 PC '이벤트' 가 늘 빈 판이었다). 고정 slug 를 **다른 고정
  //   slug 로 바꾸는 것은 답이 아니다** — 다음 캠페인 때 똑같이 깨진다.
  //   그래서 단언을 **좁힌다**: 주소 문자열이 아니라 그 주소가 하는 일 — `?event=` 가 남고,
  //   그 주소로 다시 들어가면 **같은 캠페인 판이 재현된다**(공유·새로고침 계약 자체) — 을 확인한다.
  //   옛 단언은 slug 글자만 봤을 뿐 재현되는지는 보지 않았다.
  await expect(page, '판이 떠 있는데 주소에 event 가 없다').toHaveURL((u) => u.searchParams.has('event'));
  const title = (await dlg.locator('h1').first().innerText()).trim();
  if (title !== '이벤트') {
    // 진행 중인 캠페인이 실제로 열렸다 → 옛 토큰이 **그 캠페인의 slug 로 승격**돼 있어야 한다.
    await expect(page, "'1' 이 캠페인 slug 로 승격되지 않았다 — 공유 링크가 '그때의 그 하나'를 못 가리킨다")
      .toHaveURL((u) => {
        const v = u.searchParams.get('event');
        return v !== null && v !== '1' && v !== 'true' && /^[A-Za-z0-9][A-Za-z0-9_-]{0,63}$/.test(v);
      });
  }
  //   (그 주소로 **다시 들어가면 같은 판이 재현되는지**는 바로 아래 '이벤트 새로고침' 테스트가 잠근다 —
  //    여기서 한 번 더 왕복하면 같은 계약을 두 번 재면서 병렬 실행에서 흔들리기만 한다.)
});
