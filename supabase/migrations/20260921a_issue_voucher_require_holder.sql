...
--
-- ✅ **2026-09-21 라이브 적용 완료** (오너가 이 건만 명시적으로 승인: "2 적용")
--    프로젝트 `idsxiqspecrucvfvtgbw` (NURI HOLDEM MAIN · PG 17.6) · MCP `execute_sql` 로 2부 적용.
--
--    ── 적용 전 실측(예행연습, 전부 rollback) ── 더미 매장 `dddd0000-…0001`
--      양성(인증회원 2장)=2장 OK | 🔴 NULL수신자=**1장 발급됨(구멍 열림)** | 🔴 p_count=-5 → **1장 조용히 보정됨**
--      → 저장소 파일만 보고 추정한 것이 아니라 **라이브에서 구멍을 재현**했다.
--
--    ── 적용 후 실측(예행연습, 전부 rollback) ── 비-admin 업주 매장 `f35b42d1-…`(한도 차감 분기를 타게)
--      ✅ NULL수신자 거절 | ✅ p_count=-5 거절 | ✅ p_count=1001 거절 | ✅ 없는 계정 거절
--      거절 4회 뒤 **불변: quotaΔ=0 · rowsΔ=0 · notifΔ=0**
--      ✅ 양성: 2장 반환 · **quotaΔ=-2** · rowsΔ=2  ← 정상 발급과 한도 차감은 그대로다
--
--    ── 부수 확인 ──
--      · 적용 전 라이브 정의에 `20260919a` 의 `'grant'` 사유가 있었고 **적용 후에도 남아 있다**
--        (자가검사가 그것을 단언한다) → 라이브 전용 변경을 덮어쓰지 않았음이 증명됐다.
--      · 8인자 오버로드 **정확히 1개**, 앵커 2개 모두 라이브 본문에서 1회 일치.
--      · 기존 `holder_user_id IS NULL` 행 **0건** → 이 가드가 옛 데이터와 충돌하지 않는다.
--      · ACL: anon 실행 불가 · authenticated 실행 가능(양성) 재확인.
--      · **보안 어드바이저 ERROR 0건**(WARN 3 · INFO 1 — 전부 이전부터 있던 항목).
--
--    ⚠ 재적용해도 안전하다 — 본문에 `[20260921a]` 표식이 있으면 건너뛴다.
--
-- ── 왜 ────────────────────────────────────────────────────────────────────────
