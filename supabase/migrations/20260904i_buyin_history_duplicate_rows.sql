-- 20260904i — my_buyin_history 행 중복(모든 바이인이 두 번) 수정 — claimed_names 에 distinct 복구
--
...
--   엣지 케이스가 아니라 다수 사례다.
--
-- ⚠ 이 파일의 본문은 저장소의 20260904d 가 아니라 **20260904f_buyin_history_impersonation_guard**
--   (main 9125951)의 후속이다. 20260904f 가 사칭 차단을 넣으며 my_names 를 trusted_names/claimed_names
--   2단으로 쪼갤 때 claimed_names 쪽 distinct 가 떨어져 나갔다. 그래서 20260904d 로 되돌리면 그 보안
--   강화(사칭 벡터 차단)가 통째로 사라진다 — 되돌리지 말 것.
--   본문은 적용 직전 pg_get_functiondef 로 뽑은 라이브 정의를 그대로 두고 distinct 한 단어만 되살린 것이며,
--   주석·공백을 정규화해 기계 대조한 결과 **바뀐 줄은 정확히 1줄**이다. 로직·반환 타입·컬럼명 전부 불변이고
--   **중복 행이 사라지는 것만** 달라진다.
--
--   (작업 중 실측 메모: 이 세션이 결함을 발견한 시점에는 20260904f 가 아직 main 에 없어 '라이브 정의가
--    저장소에 없다'고 판단했으나, 같은 날 9125951 로 머지된 것을 확인했다. 정본은 20260904f 다.)
--
