-- ✅ 적용 완료 (2026-09-19 라이브 실측으로 확인) — 파일에 표시가 빠져 있었을 뿐 DB 에는 들어가 있다.
--    확인 방법: pg_proc 에서 이 파일이 만드는 함수가 실제로 존재하는지 조회했다.
--    ⚠ 표시가 없으면 다음 사람이 "미적용" 으로 읽고 다시 적용하려 든다 — 그게 이번에 실제로 일어났다.
...
--
-- ══ 적용 상태 ═════════════════════════════════════════════════════════════════
-- ✅ **2026-09-18 라이브 적용 완료** (오너 승인: "지금 적용해"). MCP execute_sql 로 직접 적용.
--    적용 직후 실측 8항목 — 전부 정상:
--      함수 존재 2개 · anon 실행권 request 막힘 · anon 실행권 decide 막힘
--      authenticated 실행권 request 있음(정상)
--      search_path 고정 둘 다 `public, pg_temp`
--      🔴 봉쇄된 구 함수(request_voucher_credit)는 **그대로**다 — 이 파일이 건드리지 않았음을 확인
--      기존 voucher_credit_requests 행 수 0 (적용이 데이터를 만들지 않았다)
--    ⚠ 되돌리려면 맨 아래 ROLLBACK 블록의 drop 두 줄. 기존 봉쇄는 그대로 남는다.
