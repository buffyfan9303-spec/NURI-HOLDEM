-- ⛔ 미적용 — 오너 결정(2026-09-21, 12-ⓑ): 설계만 남기고 지금 적용하지 않는다.
-- 적용하려면 오너 승인 + .claude/skills/nuri-migration/SKILL.md 게이트가 먼저다.
-- ⚠ 이 파일이 '한 일처럼 보이는 안 한 일'이 되지 않게, 적용 전까지 머리말의 ⛔ 를 지우지 마라.
--    (전례: '마이그레이션 5개 승인 대기'가 2026-09-06 부터 상태로 박혀 있었다.)
