// ⛔ 보존본 — 배포하지 마라. 이것은 실행되는 함수가 아니다.
//
// 2026-09-21, 요구 `10-ⓑ`(오너 결정)로 `gto-explain` 에 410 거절 스텁을 배포하기 **전에**
// 라이브에 떠 있던 본문(version 9)을 그대로 떠 둔 것이다.
// 저장소에서 소스가 지워진 채 라이브만 ACTIVE 로 남아 있어서, 되돌릴 근거가 없으면
// 스텁 배포가 곧 원본 소실이 된다 — 그걸 막으려고 먼저 보존한다.
//
// 라이브 메타(보존 시점): slug=gto-explain · version=9 · status=ACTIVE · verify_jwt=true
//   ezbr_sha256 = 6997c0b531d5abeea65097d7b09d85d3568fc4f2489907eaadc6c41791385086
//   created 1780428637834 · updated 1788314786806
//
// 되살리려면: 아래 본문을 supabase/functions/gto-explain/index.ts 로 옮기고 재배포한다.
//   단 되살리기 전에 GEMINI_API_KEY 시크릿·consume_ai_quota RPC·모델 체인이 아직 유효한지 확인하라
//   (본문 주석이 적어 뒀듯 gemini-1.5-flash 는 이미 은퇴해 404 로 조용히 죽어 있던 전례가 있다).
// ──────────────────────────────────────────────────────────────────────────────
/*
// @ts-nocheck
