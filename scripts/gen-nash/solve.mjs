...
export function solve(k, S, ante, iters = ITERS) {
  ...
  // ✅ **해결됨 — `S` 는 "앤티를 낸 뒤 남은 스택" 이다.** 오너 확답 2026-09-19:
  //   "BB 앤티는 빅에서 나가는거야. 앤티가 나가고 남은 스택으로 계산."
  //
  //   (2026-09-19 검토에서 "`potWith('BB')` 는 앤티 A 를 팟에 넣는데 `callCost('BB')` 는 BB 스택에서
  //    A 를 빼지 않는다 — S 의 정의가 어디에도 없어 판정 못 한다" 는 지적이 있었다. 그 답이 이것이다.)
  //
  //   그 정의로 아래 두 줄을 검산하면 **자기정합적이다**:
  //     · BB 는 앤티 A 를 이미 냈고 남은 것이 S → 거기서 블라인드 1 을 내고, 콜에 `S−1` 을 더 넣어 총 S
  //     · 팟 `2S + 0.5 + A` = 히어로 S + BB S + SB 데드 0.5 + 앤티 A
  //     · 전원 폴드 시 히어로 순이익 `1.5 + A` = SB 0.5 + BB 1 + 앤티 A
  //   → 우려했던 "BB 콜 EV 를 (1−E) 만큼 과소평가 → 셔브 넓게·BB 콜 좁게" 치우침은 **없다.**
  //
  //   ⚠ 남는 미세한 것 하나(치우침 아님, 표기 문제): 화면의 "N bb" 는 이 S, 즉 **앤티 낸 뒤 실효 스택**이다.
  //     전원이 같은 칩으로 시작했다면 BB 만 A 를 더 내므로 올인 금액은 BB 의 남은 스택으로 캡된다 —
  //     그래서 S 를 실효 스택으로 읽는 것이 맞다. 다만 "내 칩이 12bb" 인 사용자가 12bb 표를 보는 것과
  //     "앤티 내고 12bb 남았다" 는 반 칸 다르다. 표 자체는 옳고, 라벨 해석의 문제다.
