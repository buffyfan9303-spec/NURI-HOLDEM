...
| 에이전트 | 모델 | 쓰는 때 |
|---|---|---|
| `home-team` | sonnet | 홈·전역 셸·App.tsx·index.css·atoms·알림·로그인·법적 고지 |
| `community-team` | sonnet | 게시판·댓글·외치기·신고/제재·차단·그룹·커뮤니티 광고 |
| `store-team` | sonnet | my-store·장부·클락·직원/급여·정산·이용권·순위 |
| `gto-team` | sonnet | NURI SPOT·GTO·레인지·Nash·계산기·드릴 |
| `design-reviewer` | **fable-5-1** | 화면·이미지를 실제로 열어 반응형·접근성·CLS·모션·터치 검증(기본 읽기 전용) |
| `root-cause-debugger` | **fable-5-1** | 재현 불가·첫 재발·flaky·교차 계층·원인 불명(기본 읽기 전용) |
| `critical-reviewer` | **fable-5-1** | 보안·권한·이용권·GTO 계산 독립 검토(기본 읽기 전용) |
| `capability-steward` | haiku | 필요한 스킬·플러그인·MCP·커넥터 준비 상태 |
| `verifier` | sonnet | 구현 후 독립 검증 |
| `nuri-lead` | **opus-5** | 배정·조정 |
...
### 모델 라우팅과 팀 — 2026-09-13 갱신

정본은 `.claude/rules/nuri-team-capabilities.md`다. 모델 배정·네이티브 팀·도구 점검·실행문 작성 시 읽는다.
단순 목록/기계 작업은 Haiku, 명확한 저위험 구현은 Sonnet 5, 리드 조정은 Opus 5를 사용한다.
중요한 디자인/이미지 해석·보안/권한·이용권 상태·GTO 계산·복잡한 연동·첫 재발은 처음부터 Fable 5.1에 배정한다.
기존 ‘3회차에만 Fable’ 지침을 대체한다. 실제 모델을 관찰해 기록하고 추가 결제·공급자 제한을 우회하지 않는다.
