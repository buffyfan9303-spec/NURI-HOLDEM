...
import { loadVenueVoucherPanel } from './venueVoucherLoad';
...
function harness() {
  ...
  const state = { list: [] as V[], listErr: null as unknown, loading: false, stats: null as unknown, statsErr: null as unknown, profiles: [] as unknown[], approved: true, approvedErr: null as unknown };
  const pending = new Map<string, { list: ReturnType<typeof deferred<V[]>>; stats: ReturnType<typeof deferred<unknown>>; profiles: ReturnType<typeof deferred<unknown[]>>; approved: ReturnType<typeof deferred<boolean>> }>();
  const api = {
