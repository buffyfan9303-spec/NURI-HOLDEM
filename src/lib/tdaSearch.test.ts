...
describe('AI 근거 묶음', () => {
  ...
  // 2026-09-17: 여기 있던 `it('결과가 없으면 빈 문자열 …')` 는 **본문이 빈 껍데기**였다.
  //   cd620a1(2026-09-11 'AI 기능 제거')이 단언을 지우면서 제목만 남겼고, 그 뒤 6일간 초록으로 통과했다.
  //   같은 내용은 바로 위 '검색이 0건이면 넘길 키도 0개다' 가 이미 재고 있으므로 중복 껍데기를 지운다.
  //   ⚠ 이 부류는 이제 `vitest.config.ts` 의 `expect.requireAssertions` 가 자동으로 막는다.
});
