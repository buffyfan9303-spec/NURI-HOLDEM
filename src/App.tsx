      ...
      {(activeTab === 'browse' || visitedTabs.has('browse')) && (
        <main data-tab="browse" className="tab-pane" style={activeTab !== 'browse' ? { display: 'none' } : undefined}
          ...
          {(browseNotices.length > 0 || isAdmin || !noticesLoaded) && (
                  /* 🔴 2026-09-18 오너: "공지사항과 아래 게임 내용이 곂쳐있어".
                      실측(390px): 공지 카드 하단 284.0 · 바로 아래 일정 목록 상단 284.0 — **간격 0px** 이었다.
                      겹친 것은 아니지만 둥근 카드 둘이 선 하나를 공유해 **한 덩어리로 읽힌다**.
                    ⚠ pt-3 만 있고 pb 가 없던 자리다. 위아래를 같은 값으로 맞춘다(12.75px — 루트 17px 기준 0.75rem). */
