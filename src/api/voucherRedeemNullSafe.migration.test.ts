// 이용권 사용 RPC 의 fail-open(N04 감사, 적대 반증 생존 · high) — 20260912b 보강이 SQL 에 적혀 있는가 (2026-09-13)
//
...
//   그 주장이 NULL 경로를 놓쳤다. 이 파일은 보강된 마이그레이션이 ① NULL 명시 체크 + is distinct from ② REVOKE/GRANT ③ search_path pg_temp
//   ④ 헤더의 정정을 담고 있는지를 잠근다. 2026-09-13 운영 적용 완료, 활성화는 별도다.
...
describe('20260912b(보강) — 이용권 사용 RPC 는 비로그인에서 열리지 않는다', () => {
  ...
  it('🔴 QR 매장 ID 비교도 NULL-safe 다 — null 로 매장 증빙을 우회하지 못한다', () => {
    const b = bodyOf('redeem_my_voucher_by_qr');
    expect(b).toContain('v_venue is distinct from p_venue_id');
    expect(b).not.toMatch(/v_venue\s*<>\s*p_venue_id/);
  });
  it('🔴 헤더가 옛 주장("실질 피해 없음")을 정정하고 운영 적용 상태를 적는다', () => {
    const head = SQL.slice(0, 4000);
    expect(head).toMatch(/NULL 경로를 놓쳤다/);
    expect(head).toMatch(/fail-open/);
    expect(head).toMatch(/실피해 0/);
    expect(head).toMatch(/APPLIED 2026-09-13/);
