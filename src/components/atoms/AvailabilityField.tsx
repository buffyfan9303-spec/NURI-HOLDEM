...
export function availabilityHint(status: AvailStatus, noun: string, invalidText: string): { text: string; cls: string } | null {
  switch (status) {
    case 'checking':  return { text: '확인 중…',                    cls: 'text-ink-muted' };
    case 'available': return { text: `사용 가능한 ${noun}입니다`,     cls: 'text-emerald-400' };
    case 'taken':     return { text: `✗ 이미 사용 중인 ${noun}입니다`, cls: 'text-danger' };
...
export default function AvailabilityField({
  value, status, onChange, label, noun = label, subLabel, placeholder, maxLength, invalidText,
}: {
  value: string; status: AvailStatus; onChange: (v: string) => void;
  label: string; noun?: string; subLabel?: string; placeholder?: string; maxLength?: number; invalidText: string;
}) {
  const h = availabilityHint(status, noun, invalidText);
  return (
    <div>
