...
// 사전 79개 용어가 '패널을 연 사람'에게만 닿고 계산기 UI 에는 툴팁이 0건이라
// (초보가 계산기를 닫는 이유는 숫자가 아니라 MDF·SPR·블로커 같은 '단어'다)
// Term 컴포넌트가 같은 데이터를 참조해야 했다. **복사 금지 — 여기가 유일한 원본이다.**
// GlossaryPanel 은 이 파일을 import 해서 그대로 렌더한다(내용·순서·문구 무변경).
//
// §28 준수: 설명 전반에서 '환전·현금·수익' 계열 단어를 배제하고 참가비·상금·기대값으로 통일.

export type GlossaryCat = '기본' | '액션' | '핸드' | '보드' | '수치' | '토너먼트' | '전략';
...
export const GLOSSARY_CATS: GlossaryCat[] = ['기본', '액션', '핸드', '보드', '수치', '토너먼트', '전략'];
...
// ── 조회 인덱스 ──────────────────────────────────────────────────────────────
// Term 컴포넌트가 키(한글 용어 또는 영문 표기)로 항목을 찾는다.
// 대소문자·좌우 공백만 무시하고, 그 외 정규화는 하지 않는다 —
// 느슨한 매칭(부분 문자열·자동 치환)은 오탐을 만들기 때문에 **정확 일치만** 허용한다.
const INDEX: Map<string, GlossaryEntry> = (() => {
  const m = new Map<string, GlossaryEntry>();
  const put = (k: string, e: GlossaryEntry) => { const key = k.trim().toLowerCase(); if (key && !m.has(key)) m.set(key, e); };
  for (const e of GLOSSARY_TERMS) {
    put(e.term, e);
    put(e.en, e);
    // 'SB (Small Blind)' → 'sb', 'Open (Open Raise)' → 'open' 처럼 괄호 앞 약어도 키로 받는다.
    const short = e.en.split('(')[0];
    if (short !== e.en) put(short, e);
  }
  return m;
})();

/** 사전에서 한 항목을 찾는다. 없으면 undefined — 호출부는 원문을 그대로 렌더해야 한다. */
export function findGlossaryTerm(key: string): GlossaryEntry | undefined {
