// src/components/features/VoucherManageModal.tsx — 매장이용권 관리(업주 발급 / 인증직원 열람·사용 / 사용처 분석).
import { useEffect, useState } from 'react';
import Modal from '../atoms/Modal';
import { useToast } from '../atoms/Toast';
import { useAuth } from '../../contexts/AuthContext';
import { listVenueVouchers, issueVoucher, redeemVoucher, revokeVoucher, findUserForTransfer, voucherUsageByVenue, type Voucher, type VoucherUsage } from '../../api/vouchers';
import { wonToMan } from '../../api/ledger';

const STATUS: Record<string, { label: string; cls: string }> = {
  active: { label: '발급됨', cls: 'bg-gold-300/15 text-gold-300' },
  used: { label: '사용완료', cls: 'bg-surface-float text-ink-muted' },
  revoked: { label: '취소', cls: 'bg-danger/15 text-danger-light' },
...
export default function VoucherManageModal({ open, onClose, venueId }: { open: boolean; onClose: () => void; venueId: string }) {
  ...
  useEffect(() => { if (open) reload(); /* eslint-disable-next-line */ }, [open, venueId]);
  ...
  const issue = async () => {
    ...
    try {
      ...
      await issueVoucher(venueId, { title, amount, holderName, holderUserId });
      toast.show('매장이용권을 발급했습니다', 'success');
      setTitle('매장이용권'); setAmount(0); setNick(''); setRecvName('');
      reload();
    } catch (e) { toast.show(e instanceof Error ? e.message : '발급 실패', 'error'); }
    setBusy(false);
  };
  ...
  const revoke = async (id: string) => { if (!window.confirm('이 이용권을 취소할까요?')) return; try { await revokeVoucher(id); reload(); } catch (e) { toast.show(e instanceof Error ? e.message : '실패', 'error'); } };
      ...
      <div className="space-y-3 p-4">
        {canIssue ? (
          <div className="space-y-1.5 rounded-input border border-gold-400/30 bg-gold-300/[0.05] p-2.5">
            <p className="text-2xs font-bold text-gold-300">새 매장이용권 발급 <span className="font-normal text-ink-muted">· 업주 전용</span></p>
            <div className="flex gap-1.5">
              <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="이용권 이름" className="input min-w-0 flex-1 text-sm" />
              <div className="relative w-28 shrink-0">
                <input type="number" inputMode="numeric" value={amount || ''} onChange={(e) => setAmount(parseInt(e.target.value, 10) || 0)} placeholder="금액" className="input w-full pr-7 text-sm tabular-nums" />
            ...
            <button type="button" disabled={busy} onClick={issue} className="btn-primary w-full text-sm disabled:opacity-50">{busy ? '발급 중…' : '+ 발급'}</button>
            <p className="text-[10px] text-ink-muted">닉네임 입력 시 그 회원 지갑으로 전송(앱에서 열람·전송 가능). 비우면 매장 내 사용용으로 기록.</p>
          </div>
        ) : (
          <p className="rounded-input border border-border-subtle bg-surface-low p-2.5 text-2xs text-ink-muted">발급은 <b className="text-ink-secondary">업주</b>만 가능합니다. 인증 직원은 열람·사용 처리만 할 수 있습니다.</p>
        )}

        {canIssue && usage.length > 0 && (
          <div className="rounded-input border border-border-subtle bg-surface-low p-2.5">
            <p className="mb-1 text-2xs font-bold text-ink-secondary">사용처 TOP — 발급분이 실제 사용된 매장</p>
            <ul className="space-y-1">
              {usage.slice(0, 6).map((u, i) => (
                <li key={u.usedVenueId ?? i} className="flex items-center justify-between text-2xs">
                  ...
                  <span className="shrink-0 font-bold text-ink-primary tabular-nums">{u.usedCount}건{u.totalAmount > 0 ? ` · ${wonToMan(u.totalAmount)}만` : ''}</span>
                </li>
              ))}
            </ul>
          </div>
        ...
        <div>
          <p className="mb-1 text-2xs font-bold text-ink-secondary">발급됨 {active.length}장</p>
          {loading ? <p className="py-3 text-center text-2xs text-ink-muted">불러오는 중…</p>
            : active.length === 0 ? <p className="py-3 text-center text-2xs text-ink-muted">발급된 이용권이 없습니다.</p>
              : <ul className="space-y-1.5">{active.map((v) => <Row key={v.id} v={v} onRedeem={() => redeem(v.id)} onRevoke={canIssue ? () => revoke(v.id) : undefined} />)}</ul>}
        </div>

        {others.length > 0 && (
          <div>
            <p className="mb-1 text-2xs font-bold text-ink-muted">이력 {others.length}건</p>
            <ul className="space-y-1.5">{others.slice(0, 30).map((v) => <Row key={v.id} v={v} />)}</ul>
          </div>
        )}
      </div>
    </Modal>
...
function Row({ v, onRedeem, onRevoke }: { v: Voucher; onRedeem?: () => void; onRevoke?: () => void }) {
  ...
  return (
    <li className="flex items-center gap-2 rounded-input border border-border-subtle bg-surface-low px-3 py-2">
      ...
      {onRevoke && <button type="button" onClick={onRevoke} aria-label="취소" className="shrink-0 px-1 text-xs text-ink-muted hover:text-danger-light">✕</button>}
    </li>
  );
}
