...
describe('🔴 근거 없는 긴박감 — 예약 인원만으로 "마감 임박"을 붙이지 않는다', () => {
  ...
  // 🔴 2026-09-18(5차) 개정 — 종전엔 `function statusBadge(...)` 의 인자 목록에 reserveCount 가
  //   없는지만 봤다. 오너가 상태 표시를 없애라고 해서 **그 함수 자체가 사라졌고**, 검사는
  //   "정의를 찾지 못했다" 로 빨개졌다. 함수 이름에 매달린 검사는 구조가 바뀌면 같이 죽는다.
  //   → 지키려던 **뜻**(예약 인원으로 긴박감을 만들지 않는다)을 어휘와 사용처로 직접 잠근다.
  it('예약 인원을 긴박감으로 바꾸지 않는다 — 사실 그대로만 쓴다', () => {
    // 잴 것이 실제로 있는지 먼저 확인한다(빈 통과 방지).
    expect(CARD, "'예약 N명' 표기가 사라졌다 — 이 검사가 볼 대상이 없어졌다")
      .toMatch(/예약 \{reserveCount\}명/);

    // Schedule 에는 정원(capacity)도 예약 마감 시각도 없다 — 어떤 숫자도 '임박'의 근거가 못 된다.
    const 긴박 = ['마감 임박', '임박', '서두르', '곧 마감', '매진', '얼마 안 남'];
    const 걸린것 = 긴박.filter((w) => CARD.includes(w));
    expect(걸린것, `근거 없는 긴박감 어휘가 카드에 들어왔다: ${걸린것.join(', ')}`).toEqual([]);

    // 예약 수는 '있으면 보여준다'(> 0) 외의 임계값 비교에 쓰이면 안 된다 — 10명이 곧 '임박'이었다.
    // ⚠ `[^)\n]` 로 쓰면 안 된다 — 실제 코드가 `(reserveCount ?? 0) >= 10` 이라 괄호가 중간에 낀다.
    //   처음에 그렇게 썼다가 음성 대조에서 **심어 놓은 임계값 10 을 못 잡았다**(2026-09-18).
    const 비교 = [...CARD.matchAll(/reserveCount[^\n]{0,24}?[<>]=?\s*(\d+)/g)]
      .map((m) => m[0]).filter((x) => !/>\s*0\b/.test(x));
    expect(비교, `예약 인원을 임계값과 비교한다 — 정원 데이터가 없으므로 근거가 없다:\n${비교.join('\n')}`).toEqual([]);
  });
});
