...
- 서비스 이용 과정에서 생성: 접속기록, 기기/브라우저 정보, 이용내역(장부·예약·매장이용권·매장 QR 체크인 등), Google Analytics 쿠키(이용 통계 — 브라우저 설정으로 거부 가능)
...
- 회사는 서비스 제공에 필요한 범위에서 다음과 같이 개인정보를 국외로 이전(전송·처리)합니다. 각 항목의 이전은 서비스 이용 시 관련 기능 실행과 함께 이루어집니다.
  ...
  ④ Plus Five Five, Inc.(Resend) (미국) — 항목: 이메일 주소·닉네임 / 목적: 주간 소식 이메일 발송(수신 동의자 한정) / 보유: 발송 처리 후 벤더 정책에 따른 단기 보관 / 방법: 발송 시 API 전송
  ...
  ⑥ Google LLC (미국) — 항목: NURI SPOT 에 저장한 스팟의 핸드 정보(자리·스택·카드·액션·보드) 및 이용자가 작성한 메모(최대 300자). 이름·닉네임·연락처 등 계정 식별 정보는 전송하지 않음 / 목적: NURI SPOT 'AI 아쉬운 포인트' 코칭 생성 / 보유: 본인만 열람할 수 있도록 서비스 DB에 저장, 회원 탈퇴 시 관련 계정과 함께 삭제 / 방법: 해당 기능 실행 시 암호화 통신(API)으로 전송
