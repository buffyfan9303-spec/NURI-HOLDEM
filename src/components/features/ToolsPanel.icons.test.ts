// GTO 탭 도구 카탈로그 아이콘 게이트(2026-09-03) — ① 모든 도구는 서로 다른 lucide 아이콘 ② 모두 Icon 아톰이 아는 이름.
...
describe('ToolsPanel 도구 아이콘', () => {
  it('모든 도구가 서로 다른 아이콘을 쓴다', () => {
    // 도구 수는 TOOLS 배열에서 센다(리터럴 26 은 오답 노트 추가로 27 이 되며 깨졌다) — 게이트는 '중복 0' 이다.
    const toolCount = (toolsBlock.match(/\{ key: '/g) ?? []).length;
    expect(toolCount).toBeGreaterThanOrEqual(26);
    expect(toolIcons).toHaveLength(toolCount);
