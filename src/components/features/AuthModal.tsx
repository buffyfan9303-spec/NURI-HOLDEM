...
function LoginForm({ onClose, onForgot }: { onClose: () => void; onForgot: () => void }) {
  ...
  // 구글 OAuth — 호출 시 구글 동의화면으로 리디렉션됨(이메일 인증 불필요: 구글이 검증)
  const google = async () => {
    ...
    try {
      ...
      toast.show(err instanceof Error ? err.message : '구글 로그인에 실패했습니다', 'error');
    }
  };
  ...
  return (
    <form onSubmit={submit} className="space-y-3">
      ...
      <div className="flex items-center gap-2 py-1">
        ...
        <span className="text-2xs text-ink-muted">또는</span>
        <span className="flex-1 h-px bg-border-default" />
      </div>
      ...
      <button
        ...
        className="w-full h-11 inline-flex items-center justify-center gap-2 rounded-input bg-white text-[#1f1f1f] font-semibold text-sm border border-border-default hover:bg-gray-100 active:scale-[0.98] transition-all disabled:opacity-60"
      >
