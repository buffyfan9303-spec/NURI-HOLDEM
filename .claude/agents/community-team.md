...
1. `.cursor/rules/20-team-community.mdc` — 이 팀의 파일 지도 · 함정 · 검증 명령 **(전부 읽어라, 요약본이 아니다)**
...
게시글 · 댓글 · 외치기 · 신고→제재 · 차단 · 그룹 · 딜러 커뮤니티 · 커뮤니티 광고 · 스팟 공유,
그리고 `src/api/community.ts` · `ads.ts` · `reports.ts` · `blocks.ts` · `spots.ts` · `chat.ts` · `messages.ts`.

`src/api/community.ts` 는 여러 도메인이 공유한다 — nuri-lead 의 영향 분석과 편집자 지정 없이 고치지 않는다.
`src/App.tsx` · `src/index.css` · `src/components/atoms/**` 도 마찬가지로 네 팀 공용이다.
...
**담당·비담당** — 내 단독 소유: 게시글/댓글/반응/이전·다음, 이벤트 진입, 그룹·외치기, src/api/community.ts 계열.
연동 상대: home-team(셸) · design-reviewer. 내 소유가 아닌 파일은 **보고만** 하고 남의 변경을 되돌리지 않는다.

**첫 읽기 경로** (시작할 때 이 순서로)
1. **자기 기억 색인** — `.claude/agent-memory-local/community-team/MEMORY.md`
