...
### 기준선 (2026-09-13 17:54 KST 최종)
- branch `main` · HEAD **`4b15812`** · `origin/main` 대비 **ahead 7, behind 0** · 작업 트리 **0**(깨끗)
- push 될 커밋 **7개**:
```
4b15812 fix(clock): TV 보드 블라인드가 이웃 글자와 겹치고 세로 TV 에서 잘리던 것
3daebd5 docs(handoff): 병합 후 독립 검증 PASS + 리드 과장 1건 정정
98e8c5e docs(handoff): 배포 직전 대기 상태 기록 — push 만 남았다
1862264 fix(clock): 병합이 되살린 복제본 3개
2386105 Merge origin/main — 상류 클락 리팩터(ClockStage)와 이번 단일출처 통합을 합친다
1516e2e fix(app): 실패를 '없음'으로 그리던 곳들 + 브랜드 폰트 활성화 + 게시글 전체화면
ceb4c9e docs(qa,skills): 2026-09-13 전 도메인 점검 증거표 + 검증 스킬 11종
```

### 최종 게이트 (마지막 수정 후 전량 재실행)
| 게이트 | 결과 |
|---|---|
| `npx vitest run` | ✅ 185 files / **2,057 tests** exit 0 |
| `tsc` · `eslint` · `build` · `legal:check` | ✅ 전부 0 |
| `npm run bundle:budget` | ✅ **256.6 / 257** KB gz |
| E2E main (4173 프로덕션 빌드) | ✅ **525 passed / 1 failed / 26 skipped** |
| E2E boot (직렬) | ✅ **2 passed** |

### 마지막에 닫은 결함 — TV 보드 블라인드 (4b15812)
배포 직전 독립 검증이 **직접 측정**으로 찾았다. `clock-visual.spec` 은 이 부류를 못 본다 —
big-blinds 케이스가 **타이머 폭 안정성만** 재고 블라인드 텍스트의 이웃 교차·뷰포트 잘림은 안 잰다.
(리드가 "29 passed 니 폰트 문제 없다" 고 닫으려 했던 것이 이 사각이었다.)

증상: `ClockStage` CURRENT/NEXT 가 `clamp(26px, 7.2cqmin, 128px)` 로 **높이에만 묶여** 칸 폭에 안 묶였다.
1920×1080 15K/30K 에서 NEXT 와 8px 겹침·프라이즈 열 30px 침범, 1080×1920 에서 좌측 16px 잘림.
**기존 결함**이나(폴백 폰트에서도 6자리는 24px 겹침) 폰트 활성화가 약 4배로 키우고 범위를 15K/30K 급까지 내렸다.

수정: `fontSize` 를 칸 폭에도 묶었다. `em` 은 보드 DOM 에서 **실측**(숫자 0.668em·쉼표 0.2915em·'/' ≈0.19em),
검산 414px = 검증자 독립 실측 414 ✓. 4자리는 `min` 이 종전 값을 골라 **414px 그대로**.
결과 **24칸 전부 0**, 폰트 OFF 도 0 → 기존 결함까지 닫혔다.
회귀 검사 `e2e/clock-blinds-fit.spec.ts` 10건(4자리 불변을 잠그는 2건 포함).

**미검증**: 운영자 16:9 미리보기 박스(600×338) 실측치 없음.
