...
| 번들 | `npm run bundle:budget` | 🔴 **수치를 여기 베끼지 마라.** 예산 정본은 `bundle-budget.json` 하나이고, 값은 커밋마다 움직인다 — 여기 적어 둔 표가 2026-09-15 값(JS 1000)에 멈춰 있는 동안 정본은 1014 로 두 번 올라갔고, 그 8일간 이 표를 근거로 판단하면 틀렸다. 지금 값은 `cat bundle-budget.json` 으로 읽어라. 🔴 **여유가 가장 적은 곳은 첫 화면이 아니라 JS 전체와 최대 청크다** — 이 판정은 값이 아니라 구조라 유효하다. |
