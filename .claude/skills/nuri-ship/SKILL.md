...
4. **번들 예산** — `npm run bundle:budget` (= `node scripts/bundle-budget.mjs`. **3번 직후에**, 같은 dist 로 잰다)
   ...
   `첫 화면 임계 경로 256 / 256 KB gz (여유 0%)` · `JS 전체 970.6 / 1000 (3%)` · `CSS 전체 31.3 / 32 (2%)` · `최대 청크 110.6 / 114 (3%)`.
...
| 게이트 | 명령 | 실측 |
|---|---|---|
| 린트 | `npm run lint` | `✖ 221 problems (0 errors, 221 warnings)` — **error 0 이 게이트**. 규칙별 내역까지 실측했다(아래) |
| 단위 | `npm test` | `Test Files 159 passed (159)` · `Tests 1866 passed (1866)` · 17.35s (vitest 4.1.11) |
| 빌드 | `npm run build` | **이 세션에서는 안 돌렸다** — 위 산출물 덮어쓰기 때문. 수치 없음. 돌렸으면 네가 재서 적어라 |
| 번들 | `npm run bundle:budget` | `✓ 번들 예산 통과` — 단 **첫 화면 임계 경로 256/256 KB gz = 여유 0%** (JS 970.6/1000 · CSS 31.3/32 · 최대 청크 110.6/114). 빌드 없이 기존 dist 로 잰 값이고 낡음 거부는 안 떴다 |
| E2E | `nuri-e2e` 참조 | **이 세션에서는 안 돌렸다.** 파일 수만 실측: `e2e/*.spec.ts` **83개**. 등록 규모는 `nuri-e2e` 가 `--list` 로 쟀다(main 489 + boot 2 = **491 실행**) — 수치는 거기서 봐라, 여기 복사하지 않는다 |
...
## 보고
게이트 결과를 정직하게 보고하라 — 통과는 "lint error 0 / vitest 159파일 1866테스트 / build OK / 번들 첫화면 NNN.N of 256KB / E2E N 그린"처럼 **수치로**, 실패는 출력과 함께.
