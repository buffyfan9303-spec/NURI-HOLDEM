...
> ⛔ **낡은 사본이다. 정본은 `.claude/skills/security-audit/SKILL.md` 이다.**
> 2026-09-16 실측: 이 파일 69줄 vs 정본 141줄 — **내용이 다르다.**
> 여기 적힌 게이트 기준선·수치·절차를 인용하지 마라(옛 수치를 인용하면 회귀가 통과한다).
> 고칠 것이 있으면 정본을 고쳐라. 이 파일은 남겨 두되 근거로 쓰지 않는다.
...
오너 결정 필요: …
```
AGENTS.md '보안 코딩 표준' 위반이 코드에 있으면 그 자리에서 고치되, 라이브 DB 변경은 nuri-migration 규칙(멱등·REVOKE PUBLIC·search_path·롤백)을 따른다.
